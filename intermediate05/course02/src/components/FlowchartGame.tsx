...
export function FlowchartGame() {
  ...
  const correctCount = useMemo(
    () => gameSteps.filter((s) => stepSymbols[s.id] === s.correctSymbol).length,
    [stepSymbols]
  );
        ...
        <Card className="mb-8">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div>
              ...
              <p className="text-muted dark:text-muted-dark text-sm">Level 2 : Shape Detective</p>
            </div>
            <div className="flex items-center gap-4">
              <span className="text-lg font-bold text-text dark:text-text-dark">
                ❤️ {lives}
              </span>
              <span className="text-lg font-bold text-accent">✨ {xp} XP</span>
            </div>
          </div>
          ...
          <div className="mb-6">
            <div className="h-3 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
              <motion.div className="h-full bg-gradient-to-r from-accent to-primary rounded-full" initial={{ width: 0 }} animate={{ width: `${Math.min((xp / 100) * 100, 100)}%` }} transition={{ duration: 0.5 }} />
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            {status === 'playing' && (
              <motion.div key="playing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <p className="text-center text-muted dark:text-muted-dark mb-6">প্রতিটি ধাপের জন্য সঠিক ফ্লোচার্ট সিম্বল নির্বাচন করো:</p>
                      ...
                      <div key={step.id} className="flex flex-col items-center">
                        <motion.div
                          ...
                          className={`w-full min-w-[280px] p-4 rounded-xl border-2 flex items-center gap-4 cursor-pointer transition-all duration-300 ${
                            isWrong
                              ? 'border-danger bg-red-50 dark:bg-red-900/20 animate-shake'
                              : isCorrect
                        ...
                        >
                          ...
                          <span className="font-medium text-text dark:text-text-dark flex-1">{step.text}</span>
                          ...
                          {isCorrect && <span className="text-success text-lg">✓</span>}
                          {isWrong && <span className="text-danger text-lg">✗</span>}
                        </motion.div>
                        {i < gameSteps.length - 1 && (
                          <svg className="w-5 h-5 text-muted my-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="text-center mb-6">
                  <p className="text-sm text-muted dark:text-muted-dark mb-3">সিম্বল বক্স (একটি সিম্বল ক্লিক করে তারপর ধাপে ক্লিক করো):</p>
            ...
            {status === 'success' && (
              <motion.div key="success" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center">
                ...
                <h4 className="text-2xl font-bold text-success mb-2">সঠিক!</h4>
                <p className="text-lg text-text dark:text-text-dark mb-2">🏆 Shape Detective Badge আনলক হয়েছে!</p>
                <p className="text-primary font-bold mb-6">{attempts === 1 ? '★★★ 100 XP' : attempts === 2 ? '★★ 75 XP' : '★ 50 XP'}</p>
            ...
            {status === 'failure' && (
              <motion.div key="failure" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center">
                ...
                <h4 className="text-2xl font-bold text-danger mb-2">❌ ফ্লোচার্টের রাস্তা বন্ধ হয়ে গেছে!</h4>
                <p className="text-muted dark:text-muted-dark mb-2">-1 Life</p>
                <p className="text-sm text-muted dark:text-muted-dark mb-6">শুধু ভুল সিম্বলগুলো হাইলাইট করা আছে। আবার চেষ্টা করো!</p>
