  ...
  return (
    <section ref={ref} className="py-20 px-4 bg-gray-50 dark:bg-gray-800/50">
      <div className="max-w-4xl mx-auto">
              ...
              <div className="rounded-xl overflow-hidden border-2 border-gray-300 dark:border-gray-600 min-h-[160px] flex items-center justify-center relative">
                <AnimatePresence mode="wait">
                  {step.scene ? (
                    ...
                    <p className="text-muted dark:text-muted-dark">Start to see the scene</p>
                  )}
                </AnimatePresence>
              </div>
            </div>

            <p className="text-center text-muted dark:text-muted-dark mb-6 italic">{step.desc}</p>
        ...
        {/* ── PHASE 2 ── */}
        {phase === 2 && (
          <Card className={`p-8 bg-white dark:bg-card-dark border-4 transition-colors duration-300 ${feedback === 'success' ? 'border-green-500 shadow-[0_0_24px_rgba(34,197,94,0.35)]' : feedback === 'error' ? 'border-red-500' : 'border-primary/20'}`}>
            <h3 className="text-2xl font-bold mb-4 text-center text-primary">Try Yourself</h3>
            <p className="text-center text-muted dark:text-muted-dark mb-8">ব্লকগুলো সঠিক ক্রমে সাজাও।</p>
            ...
            <div className="flex flex-col gap-3 max-w-lg mx-auto">
              {[0, 1, 2, 3].map(idx => (
                <div
                  ...
                  className={`h-14 rounded-xl border-2 border-dashed flex items-center justify-center transition-colors ${slots[idx] ? 'border-transparent' : 'border-gray-400 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 hover:border-primary'}`}
                >
                  {slots[idx] && (
                    <motion.div
                      initial={{ scale: 0.7 }}
