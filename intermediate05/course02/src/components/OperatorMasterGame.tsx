    ...
    <section ref={ref} className="py-20 px-4 bg-gray-50 dark:bg-gray-800/50">
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <p className="text-xl font-bold text-accent mb-4">Level 8 : Math Wizard</p>
        ...
        {status === 'completed' ? (
          <Card className="p-8 text-center bg-white dark:bg-card-dark border-2 border-primary/20">
            <div className="text-6xl mb-6">🎯</div>
            <h3 className="text-3xl font-bold mb-4 text-green-600 dark:text-green-400">
              অসাধারণ! তুমি গেমটি সম্পন্ন করেছ!
            </h3>
            <p className="text-xl text-muted dark:text-muted-dark mb-8">
          ...
          <Card
            className={`p-8 bg-white dark:bg-card-dark border-4 transition-colors duration-300 ${
              status === 'success'
          ...
          >
            ...
            <div className="flex justify-center items-center gap-4 bg-gray-100 dark:bg-gray-900 p-8 rounded-xl min-h-[160px] flex-wrap">
              {/* Left operand */}
              <div className="px-5 py-3 bg-gray-200 dark:bg-gray-700 rounded-lg font-mono text-2xl font-bold text-text dark:text-text-dark">
                {currentExp.left}
              </div>
              ...
              <div
                ...
                className={`w-28 h-16 rounded-xl border-2 border-dashed flex items-center justify-center transition-all duration-200 ${
                  slot
                    ? 'border-transparent'
                    : 'border-gray-400 dark:border-gray-500 bg-white/50 dark:bg-gray-800 hover:border-primary hover:bg-primary/5'
                }`}
              >
                {slot ? (
                  <motion.div
              ...
              {currentExp.result && status === 'success' && (
                <motion.div
                  ...
                  className="px-5 py-3 bg-accent/10 border-2 border-accent rounded-lg font-mono text-2xl font-bold text-accent"
                >
                  {currentExp.result}
                </motion.div>
              )}
