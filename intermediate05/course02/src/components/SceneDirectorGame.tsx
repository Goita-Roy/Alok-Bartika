    ...
    <section ref={ref} className="py-20 px-4 bg-gray-50 dark:bg-gray-800/50">
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div className="text-center mb-12" initial={{ opacity: 0, y: -20 }} animate={isVisible ? { opacity: 1, y: 0 } : {}}>
          ...
          <p className="text-xl font-bold text-accent mb-6">Level 10 : Audio-Visual Director</p>

          <div className="flex justify-center items-center gap-4 flex-wrap">
            <div className="bg-white dark:bg-card-dark px-6 py-2 rounded-full border-2 border-primary/20 shadow-sm">
              <span className="text-muted dark:text-muted-dark mr-2">XP:</span>
              <span className="font-bold text-primary">{xp}</span>
            </div>
            {/* Progress dots */}
            <div className="flex gap-2 items-center">
              {LEVELS.map((_, i) => (
                <div key={i} className={`w-3 h-3 rounded-full transition-colors ${i < levelIdx ? 'bg-green-500' : i === levelIdx && status !== 'completed' ? 'bg-primary' : i < levelIdx + 1 && status === 'completed' ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`} />
              ))}
            </div>
            {status === 'completed' && (
        ...
        {status === 'completed' ? (
          <Card className="p-10 text-center bg-white dark:bg-card-dark border-2 border-primary/20">
            <div className="text-6xl mb-6">🎬</div>
            <h3 className="text-3xl font-bold mb-4 text-green-600 dark:text-green-400">অসাধারণ! তুমি একজন Audio-Visual Director!</h3>
            <p className="text-xl text-muted dark:text-muted-dark mb-8">তুমি 🏆 Multimedia Master ব্যাজ এবং {xp} XP পেয়েছ!</p>
            <Button onClick={handleRestart} variant="primary">Play Again</Button>
          </Card>
        ) : (
          <Card className={`p-8 bg-white dark:bg-card-dark border-4 transition-colors duration-300 ${status === 'success' ? 'border-green-500 shadow-[0_0_24px_rgba(34,197,94,0.3)]' : status === 'failure' ? 'border-red-500' : 'border-primary/20'}`}>

            {/* Level header */}
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-lg font-bold text-text dark:text-text-dark">Scene {levelIdx + 1} of {LEVELS.length}</h3>
              <span className="text-sm text-muted dark:text-muted-dark font-mono">Backdrop: {currentLevel.backdrop}</span>
            </div>
            ...
            {/* Question */}
            <p className="text-center font-bold text-lg text-text dark:text-text-dark mb-8">{currentLevel.question}</p>
            ...
            <AnimatePresence mode="wait">
              {status === 'failure' && (
                <motion.div key="fail" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-center mt-4">
                  ...
                  <p className="text-muted dark:text-muted-dark text-sm mb-4">এই দৃশ্যে এই শব্দ মানায় না!</p>
