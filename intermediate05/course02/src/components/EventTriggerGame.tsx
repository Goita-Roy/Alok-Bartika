      ...
      <div className="max-w-3xl mx-auto w-full">
        ...
        <Card>
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div>
              <h3 className="text-2xl font-bold text-text dark:text-text-dark">🎮 Event Trigger Challenge</h3>
              <p className="text-muted dark:text-muted-dark text-sm">Level 3 : Event Triggerer</p>
            </div>
          ...
          <AnimatePresence mode="wait">
            {status === 'idle' && !gameStarted && (
              <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-8">
                ...
                <h4 className="text-xl font-bold text-text dark:text-text-dark mb-4">Event Trigger Challenge</h4>
                <p className="text-muted dark:text-muted-dark mb-6">সঠিক Event Block নির্বাচন করে Sprite-কে কাজ করাও!</p>
            ...
            {status === 'playing' && (
              <motion.div key="playing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <div className="bg-gradient-to-b from-blue-200 to-blue-100 dark:from-slate-700 dark:to-slate-800 rounded-xl p-8 mb-6 relative overflow-hidden min-h-[200px]">
                  <div className="absolute top-2 left-2 text-xs text-muted dark:text-muted-dark font-bold">Stage</div>
                ...
                {/* Trigger buttons */}
                <p className="text-center text-sm text-muted dark:text-muted-dark mb-4">সঠিক Event Block নির্বাচন করো:</p>
            ...
            {status === 'success' && (
              <motion.div key="success" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center">
                ...
                <p className="text-lg text-text dark:text-text-dark mb-2">{mission.emoji} {mission.action}</p>
                <p className="text-primary font-bold mb-6">🏆 Controller Master আনলক হয়েছে! +{roundAttempts === 1 ? 150 : roundAttempts === 2 ? 100 : 50} XP</p>
            ...
            {status === 'failure' && (
              <motion.div key="failure" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center">
                ...
                <h4 className="text-2xl font-bold text-danger mb-2">❌ প্রোগ্রামটি রেসপন্স করছে না</h4>
                <p className="text-muted dark:text-muted-dark mb-6">ভুল Event Block নির্বাচন করেছ। আবার চেষ্টা করো!</p>
            ...
            {status === 'idle' && gameStarted && completedMissions.size === missions.length && (
              <motion.div key="complete" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8">
                ...
                <h4 className="text-2xl font-bold text-success mb-2">সব Mission সম্পন্ন!</h4>
                <p className="text-text dark:text-text-dark mb-4">মোট {xp} XP অর্জিত!</p>
