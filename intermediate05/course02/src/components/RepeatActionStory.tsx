        ...
        <AnimatePresence mode="wait">
          {phase === 'card1' && (
            <motion.div key="c1" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} className="text-center">
              <Card>
                ...
                <p className="text-2xl font-bold text-text dark:text-text-dark mb-8">তোমাকে যদি কাগজের পাতায় ১ থেকে ১০০ পর্যন্ত সংখ্যাগুলো লিখতে বলা হয়।</p>
          ...
          {phase === 'card2' && (
            <motion.div key="c2" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} className="text-center">
              <Card>
                ...
                <p className="text-2xl font-bold text-text dark:text-text-dark mb-6">তুমি একই কলম দিয়ে বারবার ১, ২, ৩ এভাবে একই পদ্ধতিতে লিখে যাবে।</p>
          ...
          {(phase === 'card2-anim' || phase === 'card3' || phase === 'card3-fast') && (
            <motion.div key="c2a" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <Card>
                <h3 className="text-xl font-bold text-text dark:text-text-dark mb-4 text-center">লেখা হচ্ছে...</h3>
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    {visibleNumbers.map((n) => (
                      <motion.span
                        ...
                        className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white dark:bg-gray-700 shadow-sm text-sm font-bold text-text dark:text-text-dark"
                      >
                        {n}
                      </motion.span>
                    ))}
          ...
          {phase === 'card4' && (
            <motion.div key="c4" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} className="text-center">
              <Card>
                ...
                <p className="text-2xl font-bold text-text dark:text-text-dark mb-8">এই যে একই কাজ বারবার করা, একেই বলে লুপ।</p>
                <Button onClick={handleNext} size="lg">Next →</Button>
              </Card>
            </motion.div>
          )}
          ...
          {phase === 'repeat' && (
            <motion.div key="rep" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <Card>
                <h3 className="text-xl font-bold text-text dark:text-text-dark mb-4 text-center">আবার দেখো — Repeat 🔁</h3>
          ...
          {phase === 'done' && (
            <motion.div key="done" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center">
              <Card highlighted className="max-w-md mx-auto">
                ...
                <p className="text-xl font-bold text-text dark:text-text-dark">দারুণ! তুমি লুপ বুঝতে পেরেছ!</p>
              </Card>
            </motion.div>
          )}
        </AnimatePresence>
