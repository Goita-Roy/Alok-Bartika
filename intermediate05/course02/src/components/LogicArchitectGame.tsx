      ...
      <div className="max-w-3xl mx-auto w-full">
        ...
        <Card>
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div>
              <h3 className="text-2xl font-bold text-text dark:text-text-dark">🎮 Logic Architect</h3>
              <p className="text-muted dark:text-muted-dark text-sm">Level 4 : Logic Architect</p>
            </div>
          ...
          <AnimatePresence mode="wait">
            {status === 'idle' && !gameStarted && (
              <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-8">
                ...
                <h4 className="text-xl font-bold text-text dark:text-text-dark mb-4">Logic Architect</h4>
                <p className="text-muted dark:text-muted-dark mb-6">ট্রাফিক সিগন্যালের সঠিক লজিক তৈরি করো!</p>
            ...
            {status === 'success' && currentIndex < situations.length - 1 && (
              <motion.div key="success" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center py-4">
                ...
                <p className="text-xl font-bold text-success">সঠিক লজিক!</p>
              </motion.div>
            )}
            ...
            {status === 'failure' && (
              <motion.div key="failure" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center py-4">
                ...
                <p className="text-2xl font-bold text-danger mb-2">❌ Game Over</p>
            ...
            {status === 'success' && currentIndex >= situations.length - 1 && (
              <motion.div key="complete" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8">
                ...
                <h4 className="text-2xl font-bold text-success mb-2">সব লজিক সঠিক!</h4>
                <p className="text-lg text-text dark:text-text-dark mb-4">মোট {xp} XP অর্জিত!</p>
                <div className="bg-gradient-to-r from-yellow-100 to-amber-100 dark:from-yellow-900/30 dark:to-amber-900/30 border-2 border-yellow-400 rounded-xl p-4 mb-6 inline-block">
                  <p className="text-xl font-bold text-text dark:text-text-dark">🏆 Logic Architect Badge আনলক হয়েছে!</p>
                </div>
