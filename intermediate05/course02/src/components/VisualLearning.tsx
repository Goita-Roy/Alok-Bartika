...
export function VisualLearning() {
  ...
  const handleStepClick = useCallback(
    (id: number) => {
      ...
      if (clickedSteps.size === lessonData.visualSteps.length - 1) {
  ...
  const allClicked = clickedSteps.size === lessonData.visualSteps.length;

  return (
    <section
      ref={ref}
      className="min-h-screen flex items-center justify-center py-20 px-4 bg-gradient-to-br from-orange-50 to-yellow-50 dark:from-gray-900 dark:to-slate-900"
    >
      <div className="max-w-4xl mx-auto w-full">
        ...
        <div className="flex flex-col items-center gap-0 mb-12">
          {lessonData.visualSteps.map((step, i) => {
            const isClicked = clickedSteps.has(step.id) || allClicked;
            const isLast = i === lessonData.visualSteps.length - 1;
                  ...
                  className={`
                    ...
                    ${
                      isClicked
                        ...
                        : 'bg-white dark:bg-card-dark text-text dark:text-text-dark border-2 border-gray-200 dark:border-gray-600 hover:border-primary cursor-pointer'
                    }
                  `}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-text dark:text-text-dark leading-relaxed">
                      {lessonData.recipeComparison.text}
                    </p>
                  </div>
                </div>
              </Card>
