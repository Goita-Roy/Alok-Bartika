      ...
      <div className="max-w-3xl mx-auto w-full">
        ...
        <Card>
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div>
              <h3 className="text-2xl font-bold text-text dark:text-text-dark">🎮 Loop Runner</h3>
              <p className="text-muted dark:text-muted-dark text-sm">Level 5 : Infinity Coder</p>
            </div>
            <div className="flex items-center gap-4">
              <span className="text-lg font-bold text-accent">✨ {xp} XP</span>
              {penalty > 0 && <span className="text-sm text-danger">-{penalty} penalty</span>}
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            {status === 'idle' && !gameStarted && (
              <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-8">
                ...
                <h4 className="text-xl font-bold text-text dark:text-text-dark mb-4">Loop Runner</h4>
                <p className="text-muted dark:text-muted-dark mb-6">Sprite-কে স্ক্রিন জুড়ে হাঁটাতে সঠিক লুপ বেছে নাও!</p>
            ...
            {status === 'playing' && (
              <motion.div key="playing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <div className="bg-gradient-to-b from-blue-200 to-blue-100 dark:from-slate-700 dark:to-slate-800 rounded-xl p-6 mb-6 relative overflow-hidden min-h-[160px]">
                  <div className="text-xs text-muted dark:text-muted-dark font-bold">Stage</div>
                  ...
                  {/* Distance markers */}
                  <div className="absolute bottom-6 left-0 right-0 flex justify-between px-4 text-[10px] text-muted dark:text-muted-dark">
                    <span>Start</span>
                    <span>End</span>
                  </div>
                </div>

                <p className="text-center text-lg font-bold text-text dark:text-text-dark mb-6">
            ...
            {status === 'success' && (
              <motion.div key="success" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center py-8">
                ...
                <h4 className="text-2xl font-bold text-success mb-2">সঠিক লুপ!</h4>
                <p className="text-text dark:text-text-dark mb-2">Sprite অনবরত হাঁটতে থাকবে!</p>
                <p className="text-primary font-bold mb-4">🏆 Infinity Coder Badge আনলক হয়েছে! +{attempts === 1 ? 250 : attempts === 2 ? 200 : 150} XP</p>
            ...
            {status === 'failure' && (
              <motion.div key="failure" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-center py-8">
                ...
                <h4 className="text-2xl font-bold text-danger mb-2">❌ স্প্রাইট মাঝপথে আটকে গেছে</h4>
                <p className="text-muted dark:text-muted-dark mb-2">Repeat 10 শেষ হলে স্প্রাইট থেমে যায় — চলতে থাকে না!</p>
                <p className="text-sm text-danger mb-6">-50 Points</p>
