...
export function ChapterDetailsPage() {
  const { selectedChapter, course, state } = useCourse();
  const { goToSubchapter, goToHome } = useNavigation();

  useEffect(() => {
    // Selection is driven entirely by context state; nothing to sync from a URL.
  }, []);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl font-bold text-gray-900 mb-2">অধ্যায় খুঁজে পাওয়া যায়নি</h2>
          <button
            onClick={goToHome}
  ...
  const completedCount = chapter.subchapters.filter((sub) =>
    state.completedSubchapters.has(sub.id),
  ).length;
  const remainingCount = chapter.subchapters.length - completedCount;
  const isCompleted = completedCount === chapter.subchapters.length;
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-3 gap-3 sm:gap-4 mt-4 sm:mt-6">
              <div className="bg-white/10 rounded-xl sm:rounded-2xl p-3 sm:p-4 backdrop-blur-sm">
                <p className="text-xl sm:text-2xl font-bold">{toBengaliDigits(chapter.subchapters.length)}</p>
                <p className="text-[10px] sm:text-xs text-white/70 mt-0.5">মোট পাঠ</p>
              </div>
              <div className="bg-white/10 rounded-xl sm:rounded-2xl p-3 sm:p-4 backdrop-blur-sm">
                <p className="text-xl sm:text-2xl font-bold text-green-200">{toBengaliDigits(completedCount)}</p>
                <p className="text-[10px] sm:text-xs text-white/70 mt-0.5">সম্পন্ন</p>
              </div>
              <div className="bg-white/10 rounded-xl sm:rounded-2xl p-3 sm:p-4 backdrop-blur-sm">
                <p className="text-xl sm:text-2xl font-bold text-amber-200">{toBengaliDigits(remainingCount)}</p>
                <p className="text-[10px] sm:text-xs text-white/70 mt-0.5">বাকি</p>
              </div>
            </div>
          </div>
        </motion.div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mt-2 sm:mt-3">
                      <span className={`text-[9px] sm:text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                        ...
                        {isSubCompleted ? "✓ সম্পন্ন" : `পাঠ ${toBengaliDigits(subIndex + 1)}`}
                      </span>
                    </div>
