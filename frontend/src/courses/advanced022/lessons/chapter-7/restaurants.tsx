...
const LESSON_ID = "ch7-restaurants";
const SOLUTION_CODE = `class Restaurant:\n    pass\n\n\nrestaurant1 = Restaurant()\n\nprint(restaurant1)`;
...
const SUGGESTED_QUESTIONS = [
  ...
  "Restaurant Object কীভাবে তৈরি করি?",
];

const MOTIVATIONAL = [
  "চমৎকার! Restaurant Class তৈরি হয়েছে!",
  "দারুণ! তুমি পারবে!",
  "অসাধারণ চেষ্টা!",
  "তুমি সঠিক পথে আছো!",
  "বাহ! Restaurant master!",
];

const HINTS = [
  "`class` keyword ব্যবহার করো।",
  "Class name `Restaurant` রাখো।",
  "Empty Class-এর জন্য `pass` ব্যবহার করো।",
  "Object তৈরি করতে `Restaurant()` ব্যবহার করো।",
];
...
/* ── Restaurant Blueprint Animation ── */
function RestaurantBlueprintAnimation() {
  const [step, setStep] = useState(-1);

  const steps = [
    { icon: "📐", label: "Restaurant Class", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "⬇️", label: "", color: "text-gray-400", bg: "bg-gray-800/30 border-gray-500/50" },
    { icon: "🍕", label: "Pizza House", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
    { icon: "🍔", label: "Burger Corner", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "🍜", label: "Noodle Cafe", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
    { icon: "✨", label: "One Class → Many Restaurants!", color: "text-cyan-400", bg: "bg-cyan-900/30 border-cyan-500/50" },
  ];
        ...
        {step >= 5 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              ...
              <p className="text-sm font-bold text-green-400">One class creates many restaurant objects!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function LearningFlowAnimation() {
  ...
  const steps = [
    ...
    { icon: "⬇️", label: "", color: "text-gray-400", bg: "bg-gray-800/30 border-gray-500/50" },
    { icon: "🏪", label: "Restaurant Class", color: "text-cyan-400", bg: "bg-cyan-900/30 border-cyan-500/50" },
    { icon: "⬇️", label: "", color: "text-gray-400", bg: "bg-gray-800/30 border-gray-500/50" },
    { icon: "📦", label: "Create Objects", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "⬇️", label: "", color: "text-gray-400", bg: "bg-gray-800/30 border-gray-500/50" },
    { icon: "🍕", label: "Different Restaurants", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
      ...
      <AnimatePresence>
        {step >= 7 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              <span className="text-lg">🎉</span>
              <p className="text-sm font-bold text-green-400">Restaurant Class → Objects → Easy Organization!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex items-center justify-center gap-3">
        ...
        >
          <div className="flex gap-2">
            ...
            >
              <span className="text-lg font-mono font-bold text-white">Restaurant</span>
            </motion.div>
...
function ExampleSection() {
  ...
  const fullCode = 'class Restaurant:\n    pass\n\n\nrestaurant1 = Restaurant()\n\nprint(restaurant1)';
  ...
  const startDemo = useCallback(() => {
    ...
    const typeInterval = setInterval(() => {
      if (idx < fullCode.length) {
        ...
        setTimeout(() => {
          setPhase("running");
          setFlowSteps(["🏪 Creating Restaurant Class..."]);
          setTimeout(() => {
            setFlowSteps(prev => [...prev, "🍕 Creating Restaurant Object..."]);
            setTimeout(() => {
              setFlowSteps(prev => [...prev, "✅ Restaurant Ready!"]);
              setOutputLines(["<__main__.Restaurant object at 0x7f8b8c0b4d90>"]);
              setPhase("done");
            }, 600);
          }, 800);
        }, 400);
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-gray-400 text-xs font-mono">restaurant.py</span>
          </div>
        </div>
          ...
          {codeLines.map((line, i) => (
            <div key={i} className="flex">
              ...
              <div>
                ...
                ) : line.trim().startsWith("restaurant1") ? (
                  <>
                    <span className="text-cyan-400">restaurant1</span>
                    <span className="text-gray-300"> = </span>
                    <span className="text-cyan-400">Restaurant()</span>
                  </>
                ) : line.trim().startsWith("print") ? (
                  <span className="pl-4"><span className="text-green-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">restaurant1</span><span className="text-gray-300">)</span></span>
                ) : (
                  <span>{line}</span>
                )}
              </div>
        ...
        <div className="px-4 pb-3 bg-gray-900">
          <div className="space-y-1.5">
            ...
            {flowSteps.length === 0 && (
              <p className="text-[10px] text-gray-500 font-mono">🏪 Create a restaurant class to start...</p>
            )}
          </div>
        </div>
        ...
        {phase === "done" && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-bold text-green-700">The `Restaurant` Class is the blueprint. `Restaurant()` creates a new Restaurant Object. The Object is stored inside `restaurant1`!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
/* ── Main Component ── */
export function Ch7Restaurants({
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      else if (q.includes("Object") || q.includes("object")) { a = "Object হলো Class থেকে তৈরি করা একটি instance। যেমন: restaurant1 = Restaurant() — এতে restaurant1 একটি Object।"; }
      else if (q.includes("Restaurant Class")) { a = "Restaurant Class হলো একটি blueprint যেটা দিয়ে আমরা বিভিন্ন restaurant Object তৈরি করতে পারি!"; }
      else if (q.includes("pass")) { a = "`pass` keyword ব্যবহার করি যখন Class-এর ভেতরে কিছু লিখতে না হয় — এটা empty Class তৈরি করে!"; }
      else if (q.includes("কীভাবে") || q.includes("তৈরি")) { a = "Restaurant Object তৈরি করতে `Restaurant()` ব্যবহার করো। যেমন: restaurant1 = Restaurant()"; }
      else { a = "ভালো প্রশ্ন! Restaurant Class হলো blueprint, যেটা দিয়ে অনেক restaurant Object তৈরি করো!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">Restaurant Class কী?</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              <span className="font-bold text-cyan-600">Restaurant Class</span> হলো একটি <span className="font-bold text-purple-600">blueprint</span> — যেটা দিয়ে আমরা অনেক restaurant Object তৈরি করি!
            </p>
            <p className="text-gray-700 text-[15px] leading-relaxed mt-2">
              <span className="font-bold text-amber-600">Restaurant Object</span> হলো Class থেকে তৈরি করা একটি <span className="font-bold text-green-600">instance</span> —
              একই Class থেকে অনেক restaurant তৈরি করা যায়!
            </p>
          </motion.div>

          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🏪 Restaurant Class → Many Restaurants</p>
              <RestaurantBlueprintAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> Restaurant Class
                হলো blueprint —
                একবার লিখো, অনেক restaurant Object তৈরি করো!
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">Restaurant Class আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "🍕", label: "Pizza Shop", desc: "একটি design দিয়ে অনেক pizza!", code: "PizzaShop()" },
              { icon: "🍔", label: "Burger Restaurant", desc: "একটি design দিয়ে অনেক burger!", code: "BurgerRestaurant()" },
              { icon: "🍜", label: "Noodle Restaurant", desc: "একটি design দিয়ে অনেক noodle!", code: "NoodleRestaurant()" },
              { icon: "🍰", label: "Bakery", desc: "একটি design দিয়ে অনেক cake!", code: "Bakery()" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🍕 Pizza Shop Example</p>
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
                    className="px-4 py-3 bg-red-900/30 border border-red-500/30 rounded-xl">
                    <p className="text-red-400 font-mono text-sm">Make each pizza manually</p>
                  </motion.div>
                ...
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}
                    ...
                    <p className="text-green-400 font-mono text-sm">Use Restaurant Class!</p>
                  </motion.div>
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                ...
                Restaurant Class ঠিক যেমন pizza shop —
                একটি design দিয়ে অনেক restaurant Object তৈরি করো!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: Restaurant Class তৈরি করো!</p>
                  <p className="text-xs text-gray-500 mt-1">restaurant.py ফাইলে Class এবং Object তৈরি করো</p>
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-purple-400">class</span> <span className="text-cyan-400">Restaurant</span><span className="text-gray-300">:</span></p>
                <p className="pl-4"><span className="text-gray-500">pass</span></p>
                <p></p>
                <p><span className="text-cyan-400">restaurant1</span> <span className="text-gray-300">=</span> <span className="text-cyan-400">Restaurant()</span></p>
                <p></p>
                <p><span className="text-green-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">restaurant1</span><span className="text-gray-300">)</span></p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "Restaurant নামে একটি Class তৈরি করো।", code: "class Restaurant:" },
              { num: 2, text: "Class-এর ভেতরে `pass` ব্যবহার করো।", code: "    pass" },
              { num: 3, text: "restaurant1 নামে একটি Object তৈরি করো।", code: "restaurant1 = Restaurant()" },
              { num: 4, text: "Object কে Print করো।", code: "print(restaurant1)" },
            ].map((step, i) => (
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-rose-800 via-pink-800 to-purple-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 7: Classes & Objects - Restaurants</p>
                  <p className="text-white/50 text-xs mt-1">Learn about Python Restaurant Classes</p>
                </div>
              </div>
            </div>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🏆</span>
              <p className="text-sm font-bold text-purple-800">Goal: Restaurant Class তৈরি করো!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-purple-400">class Restaurant:</p>
              <p className="text-gray-500 pl-4">pass</p>
              <p></p>
              <p className="text-cyan-400">restaurant1 = Restaurant()</p>
              <p className="text-green-400">print(restaurant1)</p>
            </div>
          </motion.div>
        ...
        <div className="px-6 py-6">
          <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-2xl p-4 mb-4">
            <p className="text-sm text-indigo-800 leading-relaxed">
              Restaurant Class সম্পর্কে জিজ্ঞাসা করো! AI তোমাকে help করবে।
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <input ref={aiRef} value={aiInput} onChange={(e) => setAiInput(e.target.value)}
              ...
              placeholder="Ask about Restaurant Classes..."
