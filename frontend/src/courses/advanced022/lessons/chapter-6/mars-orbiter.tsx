...
const LESSON_ID = "ch6-mars-orbiter";
const SOLUTION_CODE = `def km_to_miles(km):\n    return km * 0.621371\n\n\ndef miles_to_km(miles):\n    return miles * 1.60934\n\n\nprint(km_to_miles(100))\nprint(miles_to_km(100))`;

const SUGGESTED_QUESTIONS = [
  "Mars Orbiter বাগ কী ছিল?",
  "Functions দিয়ে কিভাবে বাগ ঠিক করি?",
  "Return statement কী?",
  "Parameter আর Argument এর পার্থক্য?",
  "Functions দিয়ে code reuse কিভাবে হয়?",
];

const MOTIVATIONAL = [
  "চমৎকার! Mars Orbiter bug ঠিক হয়েছে!",
  "দারুণ! তুমি পারবে!",
  "অসাধারণ চেষ্টা!",
  "তুমি সঠিক পথে আছো!",
  "বাহ! Bug hunter!",
...
const HINTS = [
  "km_to_miles function লিখো।",
  "miles_to_km function লিখো।",
  "return statement ব্যবহার করো।",
  "Functions কে call করো।",
];
...
/* ── Mars Orbiter Story Animation ── */
function MarsOrbiterStory() {
  const [step, setStep] = useState(-1);

  const steps = [
    { icon: "🚀", label: "Mars Orbiter Launched!", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "🔴", label: "Reaching Mars...", color: "text-red-400", bg: "bg-red-900/30 border-red-500/50" },
    { icon: "💥", label: "Crash! Bug Found!", color: "text-yellow-400", bg: "bg-yellow-900/30 border-yellow-500/50" },
    { icon: "🔍", label: "Unit Conversion Bug", color: "text-orange-400", bg: "bg-orange-900/30 border-orange-500/50" },
    { icon: "🔧", label: "Fix with Functions!", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
    { icon: "🎉", label: "Mission Success!", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
  ];
        ...
        {step >= 5 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              ...
              <p className="text-sm font-bold text-green-400">Functions fixed the Mars Orbiter bug!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function UnitConversionDemo() {
  const [input, setInput] = useState("100");
  const [direction, setDirection] = useState<"km-to-miles" | "miles-to-km">("km-to-miles");
  const [showResult, setShowResult] = useState(false);
  const [result, setResult] = useState(0);

  const calculate = useCallback(() => {
    const num = parseFloat(input);
    if (isNaN(num)) { setResult(0); return; }
    if (direction === "km-to-miles") {
      setResult(num * 0.621371);
    } else {
      setResult(num * 1.60934);
    }
    setShowResult(true);
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <button
          onClick={() => setDirection("km-to-miles")}
          className={`flex-1 px-4 py-2 rounded-xl font-bold text-sm transition-all ${direction === "km-to-miles" ? "bg-cyan-500 text-white" : "bg-gray-700 text-gray-300"}`}
        >
          KM → Miles
        </button>
        <button
          onClick={() => setDirection("miles-to-km")}
          className={`flex-1 px-4 py-2 rounded-xl font-bold text-sm transition-all ${direction === "miles-to-km" ? "bg-purple-500 text-white" : "bg-gray-700 text-gray-300"}`}
        >
          Miles → KM
        </button>
      </div>
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 px-4 py-3 bg-gray-800 border border-gray-600 rounded-xl text-white font-mono text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400"
          placeholder="Enter value..."
        />
        <motion.button
          whileTap={{ scale: 0.95 }}
          onClick={calculate}
          className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-bold rounded-xl transition-all shadow-md"
        >
          Convert
        </motion.button>
      </div>
      ...
      <AnimatePresence>
        {showResult && (
          <motion.div
            ...
            className="bg-green-900/30 border border-green-500/30 rounded-xl p-4 text-center"
          >
            <p className="text-green-400 font-mono text-lg font-bold">
              {result.toFixed(2)} {direction === "km-to-miles" ? "miles" : "km"}
            </p>
          </motion.div>
        )}
      </AnimatePresence>
...
function ExampleSection() {
  ...
  const fullCode = 'def km_to_miles(km):\n    return km * 0.621371\n\n\ndef miles_to_km(miles):\n    return miles * 1.60934\n\n\nprint(km_to_miles(100))\nprint(miles_to_km(100))';
  ...
  const startDemo = useCallback(() => {
    ...
    const typeInterval = setInterval(() => {
      if (idx < fullCode.length) {
        ...
        setTimeout(() => {
          setPhase("running");
          setFlowSteps(["📞 Calling km_to_miles(100)..."]);
          setTimeout(() => {
            setFlowSteps(prev => [...prev, "⚡ Function Runs!"]);
            setTimeout(() => {
              setFlowSteps(prev => [...prev, "🔢 Returns: 62.1371"]);
              setOutputLines(["62.1371"]);
              setTimeout(() => {
                setFlowSteps(prev => [...prev, "📞 Calling miles_to_km(100)..."]);
                setTimeout(() => {
                  setFlowSteps(prev => [...prev, "⚡ Function Runs!"]);
                  setTimeout(() => {
                    setFlowSteps(prev => [...prev, "🔢 Returns: 160.934"]);
                    setOutputLines(prev => [...prev, "160.934"]);
                    setPhase("done");
                  }, 600);
                }, 800);
              }, 400);
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-gray-400 text-xs font-mono">conversion.py</span>
          </div>
        </div>
              ...
              <div>
                {line.trim().startsWith("def") ? (
                  <>
                    ...
                    <span className="text-gray-300">({line.trim().match(/\((\w+)\)/)?.[1]}):</span>
                  </>
        ...
        {phase === "done" && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-bold text-green-700">Functions with return values fixed the unit conversion bug!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
/* ── Main Component ── */
export function Ch6MarsOrbiter({
      ...
      for (const line of lines) {
        ...
        if (printMatch) {
          ...
          if (funcCallMatch) {
            ...
            const funcArg = parseFloat(funcCallMatch[2]);
            if (functions[funcName] && !isNaN(funcArg)) {
              ...
              if (returnLine) {
                ...
                if (numMatch) {
                  const result = funcArg * parseFloat(numMatch[2]);
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      let a = "";
      if (q.includes("Mars Orbiter") || q.includes("bug")) { a = "Mars Orbiter-এ unit conversion bug ছিল — km আর miles মিক্স হয়ে গিয়েছিল! Functions দিয়ে এটা ঠিক করা হয়েছে।"; }
      else if (q.includes("Return") || q.includes("return")) { a = "return statement function থেকে value ফিরিয়ে দেয়। যেমন: return km * 0.621371 — এতে converted value ফিরবে!"; }
      else if (q.includes("Parameter") || q.includes("parameter")) { a = "Parameter হলো function definition-এর ভেতরের variable। যেমন: def km_to_miles(km): — এখানে km একটি parameter।"; }
      else if (q.includes("Argument") || q.includes("argument")) { a = "Argument হলো function call করার সময় পাঠানো actual value। যেমন: km_to_miles(100) — এখানে 100 একটি argument।"; }
      else { a = "ভালো প্রশ্ন! Mars Orbiter bug unit conversion-এর কারণে হয়েছিল। Functions দিয়ে এটা সমাধান করা হয়েছে!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        <motion.div variants={FADE_UP} className="px-6 py-5 bg-gradient-to-r from-red-500 via-pink-500 to-rose-600">
          <div className="flex items-center gap-3"><span className="text-3xl">📘</span><h2 className="text-xl font-extrabold text-white tracking-tight">Mars Orbiter</h2></div>
        </motion.div>
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">Mars Orbiter Bug</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              <span className="font-bold text-cyan-600">Mars Orbiter</span>-এ একটি <span className="font-bold text-red-600">unit conversion bug</span> ছিল — km আর miles মিক্স হয়ে গিয়েছিল!
            </p>
            <p className="text-gray-700 text-[15px] leading-relaxed mt-2">
              <span className="font-bold text-amber-600">Functions</span> দিয়ে আমরা এটা ঠিক করতে পারি —
              একবার লিখো, যত খুশি ব্যবহার করো!
            </p>
          </motion.div>

          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🚀 Mars Orbiter Story</p>
              <MarsOrbiterStory />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🔢 Unit Conversion Demo</p>
              <UnitConversionDemo />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> Mars Orbiter bug unit conversion-এর কারণে হয়েছিল।
                Functions দিয়ে এটা সমাধান করা হয়েছে — একবার লিখো, যত খুশি ব্যবহার করো!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">Unit conversion আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "🌡️", label: "Temperature", desc: "Celsius আর Fahrenheit কনভার্ট করো!", code: "c_to_f()" },
              { icon: "⚖️", label: "Weight", desc: "KG আর Pound কনভার্ট করো!", code: "kg_to_lb()" },
              { icon: "📏", label: "Distance", desc: "KM আর Miles কনভার্ট করো!", code: "km_to_miles()" },
              { icon: "🕐", label: "Time", desc: "Hours আর Minutes কনভার্ট করো!", code: "hours_to_mins()" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🌡️ Temperature Example</p>
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
                    className="px-4 py-3 bg-red-900/30 border border-red-500/30 rounded-xl">
                    <p className="text-red-400 font-mono text-sm">Convert manually</p>
                  </motion.div>
                  <p className="text-xs text-gray-500 mt-2">❌ Error-prone!</p>
                </div>
                ...
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}
                    ...
                    <p className="text-green-400 font-mono text-sm">Use Function!</p>
                  </motion.div>
                  <p className="text-xs text-gray-500 mt-2">✅ Reliable!</p>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> Unit conversion function দিয়ে করলে error কমে যায়!
                যেমন temperature, weight, distance — সব function দিয়ে করো!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: Unit Conversion Functions!</p>
                  <p className="text-xs text-gray-500 mt-1">conversion.py ফাইলে conversion functions লেখো</p>
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-purple-400">def</span> <span className="text-cyan-400">km_to_miles</span><span className="text-gray-300">(km):</span></p>
                <p className="pl-4"><span className="text-purple-400">return</span> <span className="text-green-400">km * 0.621371</span></p>
                <p></p>
                <p><span className="text-purple-400">def</span> <span className="text-cyan-400">miles_to_km</span><span className="text-gray-300">(miles):</span></p>
                <p className="pl-4"><span className="text-purple-400">return</span> <span className="text-green-400">miles * 1.60934</span></p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "km_to_miles function লিখো।", code: "def km_to_miles(km):" },
              { num: 2, text: "Function-এর ভেতরে return statement ব্যবহার করো।", code: "    return km * 0.621371" },
              { num: 3, text: "miles_to_km function লিখো।", code: "def miles_to_km(miles):" },
              { num: 4, text: "Functions কে call করো।", code: "print(km_to_miles(100))\nprint(miles_to_km(100))" },
            ].map((step, i) => (
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={FADE_UP} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-rose-800 via-pink-800 to-purple-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 6: Mars Orbiter</p>
                  <p className="text-white/50 text-xs mt-1">Fix Unit Conversion Bug</p>
                </div>
              </div>
            </div>
            <div className="absolute top-3 right-3 px-2.5 py-1 bg-black/70 backdrop-blur-sm rounded-lg text-white text-xs font-mono">8:10</div>
            <AnimatePresence>
              {videoComplete && (
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-gray-500 w-10 text-right">8:10</span>
          </div>
        </div>
      </motion.section>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🏆</span>
              <p className="text-sm font-bold text-purple-800">Goal: Unit Conversion Functions!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-purple-400">def km_to_miles(km):</p>
              <p className="text-purple-400 pl-4">return km * 0.621371</p>
              <p></p>
              <p className="text-cyan-400">print(km_to_miles(100))</p>
            </div>
          </motion.div>
            ...
            <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
              ...
              <div className="flex bg-gray-900">
                ...
                <textarea value={editorCode} onChange={(e) => { setEditorCode(e.target.value); setShowEncourage(false); }}
                  placeholder={'def km_to_miles(km):\n    return km * 0.621371\n\n\ndef miles_to_km(miles):\n    return miles * 1.60934\n\n\nprint(km_to_miles(100))\nprint(miles_to_km(100))'}
              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="overflow-hidden">
                <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-md">
                  ...
                  <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
                    ...
                    <p className="pl-4"><span className="text-purple-400">return</span> <span className="text-green-400">miles * 1.60934</span></p>
                    <p></p>
                    <p><span className="text-green-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">km_to_miles</span><span className="text-gray-300">(</span><span className="text-green-400">100</span><span className="text-gray-300">))</span></p>
                    <p><span className="text-green-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">miles_to_km</span><span className="text-gray-300">(</span><span className="text-green-400">100</span><span className="text-gray-300">))</span></p>
                  </div>
                </div>
              </motion.div>
            )}
        ...
        <div className="px-6 py-6">
          <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-2xl p-4 mb-4">
            <p className="text-sm text-indigo-800 leading-relaxed">
              Mars Orbiter bug সম্পর্কে জিজ্ঞাসা করো! AI তোমাকে help করবে।
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <input ref={aiRef} value={aiInput} onChange={(e) => setAiInput(e.target.value)}
              ...
              placeholder="Ask about Mars Orbiter bug..."
