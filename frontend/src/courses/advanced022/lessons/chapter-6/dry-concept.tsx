...
const LESSON_ID = "ch6-dry";
const SOLUTION_CODE = `def greet(name):\n    print(f"Hello, {name}!")\n\ngreet("Alice")\ngreet("Bob")\ngreet("Charlie")`;

const SUGGESTED_QUESTIONS = [
  "D.R.Y. কী?",
  "কেন কোড রিপিট করবেনা?",
  "Function দিয়ে D.R.Y. কিভাবে পালন করি?",
  "Parameters কী?",
  "Arguments কী?",
];

const MOTIVATIONAL = [
  "চমৎকার! D.R.Y. principle মনে রাখো!",
  "দারুণ! তুমি পারবে!",
  "অসাধারণ চেষ্টা!",
  "তুমি সঠিক পথে আছো!",
  "বাহ! D.R.Y. coding!",
...
const HINTS = [
  "Function দিয়ে একই কোড বারবার না লিখে একবার লিখো।",
  "Parameters ব্যবহার করে function-কে flexible করো।",
  "একই function বিভিন্ন input দিয়ে call করো।",
  "Code reuse মানে D.R.Y.!",
];
...
function BeforeAfterAnimation() {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {showAfter ? "🔄 আগেরটা দেখান" : "▶ রিফ্যাক্টরিং দেখান"}
      </button>
      ...
      <div className="grid grid-cols-1 gap-4">
        <motion.div
          ...
          className="bg-red-900/20 border-2 border-red-500/30 rounded-2xl p-4"
        >
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xl">❌</span>
            <p className="text-sm font-bold text-red-400">Before (Repeating Code)</p>
          </div>
          <div className="bg-gray-900 rounded-xl p-3 font-mono text-xs space-y-1">
            <p className="text-gray-400"># Repeated code!</p>
            <p className="text-green-400">print("Hello, Alice!")</p>
            <p className="text-green-400">print("Hello, Bob!")</p>
            <p className="text-green-400">print("Hello, Charlie!")</p>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-green-900/20 border-2 border-green-500/30 rounded-2xl p-4"
        >
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xl">✅</span>
            <p className="text-sm font-bold text-green-400">After (D.R.Y. Principle)</p>
          </div>
          <div className="bg-gray-900 rounded-xl p-3 font-mono text-xs space-y-1">
            <p className="text-gray-400"># Reusable function!</p>
            <p className="text-purple-400">def greet(name):</p>
            <p className="text-green-400 pl-4">print(f"Hello, {"{name}"}!")</p>
            <p className="text-cyan-400">greet("Alice")</p>
            <p className="text-cyan-400">greet("Bob")</p>
            <p className="text-cyan-400">greet("Charlie")</p>
          </div>
        </motion.div>
      </div>
        ...
        {showAfter && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              ...
              <p className="text-sm font-bold text-green-400">Write once, use many times!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function ParametersAnimation() {
  const [step, setStep] = useState(-1);

  const steps = [
    { icon: "📝", label: "Write Function with Parameter", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "📞", label: "Call with 'Alice'", color: "text-cyan-400", bg: "bg-cyan-900/30 border-cyan-500/50" },
    { icon: "⚡", label: "Runs: Hello, Alice!", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
    { icon: "📞", label: "Call with 'Bob'", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
    { icon: "⚡", label: "Runs: Hello, Bob!", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
  ];
      ...
      <AnimatePresence>
        {step >= 4 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              <span className="text-lg">🎉</span>
              <p className="text-sm font-bold text-green-400">Parameters make functions flexible!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
  ...
  const startDemo = useCallback(() => {
    ...
    const typeInterval = setInterval(() => {
      if (idx < fullCode.length) {
        ...
        setTimeout(() => {
          setPhase("running");
          setFlowSteps(["📞 Calling greet('Alice')..."]);
          setTimeout(() => {
            setFlowSteps(prev => [...prev, "⚡ Function Runs!"]);
            setTimeout(() => {
              setFlowSteps(prev => [...prev, "👋 Hello, Alice!"]);
              setOutputLines(["Hello, Alice!"]);
              setTimeout(() => {
                setFlowSteps(prev => [...prev, "📞 Calling greet('Bob')..."]);
                setTimeout(() => {
                  setFlowSteps(prev => [...prev, "⚡ Function Runs!"]);
                  setTimeout(() => {
                    setFlowSteps(prev => [...prev, "👋 Hello, Bob!"]);
                    setOutputLines(prev => [...prev, "Hello, Bob!"]);
                    setTimeout(() => {
                      setFlowSteps(prev => [...prev, "📞 Calling greet('Charlie')..."]);
                      setTimeout(() => {
                        setFlowSteps(prev => [...prev, "⚡ Function Runs!"]);
                        setTimeout(() => {
                          setFlowSteps(prev => [...prev, "👋 Hello, Charlie!"]);
                          setOutputLines(prev => [...prev, "Hello, Charlie!"]);
                          setPhase("done");
                        }, 600);
                      }, 800);
                    }, 400);
            ...
            <div key={i} className="flex">
              ...
              <div>
                {line.trim().startsWith("def") ? (
                  ...
                  <span className="pl-4"><span className="text-green-400">print</span><span className="text-gray-300">(f"Hello, {"{name}"}!")</span></span>
                ) : line.trim().startsWith("greet") ? (
                  <>
                    <span className="text-cyan-400">greet</span>
                    <span className="text-gray-300">(</span>
                    <span className="text-green-400">{line.trim().match(/greet\((.+)\)/)?.[1]}</span>
                    <span className="text-gray-300">)</span>
                  </>
                ) : (
                  <span>{line}</span>
      ...
      <AnimatePresence>
        {phase === "done" && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-2">
              <span className="text-xl">✅</span>
              <p className="text-sm font-bold text-green-700">One function, many calls! That's the D.R.Y. principle in action.</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export function Ch6DryConcept({
  ...
  const handleRun = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      for (const line of lines) {
        ...
        const defMatch = t.match(/^def\s+(\w+)\s*\((\w+)\)\s*:/);
        if (defMatch) {
          funcName = defMatch[1];
          inFunc = true;
          continue;
        ...
        if (inFunc) {
          ...
          const callMatch = t.match(/^(\w+)\s*\(\s*["']([^"']+)["']\s*\)$/);
          if (callMatch && callMatch[1] === funcName) {
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      let a = "";
      if (q.includes("D.R.Y.") || q.includes("dry")) { a = "D.R.Y. মানে Don't Repeat Yourself — একই কোড বারবার না লিখে function দিয়ে reusable করো!"; }
      else if (q.includes("Parameter") || q.includes("parameter")) { a = "Parameter হলো function-এর definition-এর ভেতরের variable যেটা input নেয়। যেমন: def greet(name): — এখানে name একটি parameter।"; }
      else if (q.includes("Argument") || q.includes("argument")) { a = "Argument হলো function call করার সময় পাঠানো actual value। যেমন: greet('Alice') — এখানে 'Alice' একটি argument।"; }
      else if (q.includes("কেন") || q.includes("কেন ব্যবহার")) { a = "D.R.Y. প্রিন্সিপল মেনে চললে কোড ছোট, পরিষ্কার এবং maintenance-friendly থাকে!"; }
      else { a = "ভালো প্রশ্ন! D.R.Y. principle মানে একই কোড বারবার না লেখা। function দিয়ে এটা সম্ভব!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        <motion.div variants={FADE_UP} className="px-6 py-5 bg-gradient-to-r from-red-500 via-pink-500 to-rose-600">
          <div className="flex items-center gap-3"><span className="text-3xl">📘</span><h2 className="text-xl font-extrabold text-white tracking-tight">D.R.Y. Concept</h2></div>
        </motion.div>
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">D.R.Y. কী?</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              <span className="font-bold text-cyan-600">D.R.Y.</span> মানে <span className="font-bold text-purple-600">Don't Repeat Yourself</span> — একই কোড বারবার না লিখে, একবার লিখে ব্যবহার করো!
            </p>
            <p className="text-gray-700 text-[15px] leading-relaxed mt-2">
              Function দিয়ে আমরা D.R.Y. principle পালন করি —
              <span className="font-bold text-amber-600"> একবার লিখো, যত খুশি ব্যবহার করো!</span>
            </p>
          </motion.div>

          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🔄 Before vs After Refactoring</p>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">⚙️ Parameters in Action</p>
              <ParametersAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> D.R.Y. principle মানে একই কোড বারবার না লেখা।
                Function দিয়ে এটা সম্ভব — একবার লিখো, বারবার ব্যবহার করো!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">D.R.Y. principle আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "🍳", label: "Recipe", desc: "একই recipe বারবার লিখো না, একবার লিখে রাখো!", code: "follow_recipe()" },
              { icon: "📱", label: "Phone Contacts", desc: "নাম্বার বারবার না লিখে contact-এ save করো!", code: "call_contact()" },
              { icon: "🔧", label: "Tools", desc: "একই কাজে একই tool ব্যবহার করো, নতুন না বানাও!", code: "use_tool()" },
              { icon: "📝", label: "Templates", desc: "একই letter template বারবার ব্যবহার করো!", code: "use_template()" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🍳 Recipe Example</p>
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
                    className="px-4 py-3 bg-red-900/30 border border-red-500/30 rounded-xl">
                    <p className="text-red-400 font-mono text-sm">Write recipe again</p>
                  </motion.div>
                ...
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}
                    ...
                    <p className="text-green-400 font-mono text-sm">Follow recipe!</p>
                  </motion.div>
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> D.R.Y. principle মানে একই কাজ বারবার না করে, একবার করে রাখো!
                যেমন recipe, যেমন function!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: D.R.Y. Function লেখো!</p>
                  <p className="text-xs text-gray-500 mt-1">functions.py ফাইলে greet function তৈরি করো</p>
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-purple-400">def</span> <span className="text-cyan-400">greet</span><span className="text-gray-300">(name):</span></p>
                <p className="pl-4"><span className="text-green-400">print</span><span className="text-gray-300">(f"Hello, {"{name}"}!")</span></p>
                <p></p>
                <p><span className="text-cyan-400">greet</span><span className="text-gray-300">(</span><span className="text-green-400">"Alice"</span><span className="text-gray-300">)</span></p>
                <p><span className="text-cyan-400">greet</span><span className="text-gray-300">(</span><span className="text-green-400">"Bob"</span><span className="text-gray-300">)</span></p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "greet নামে একটি Function তৈরি করো।", code: "def greet(name):" },
              { num: 2, text: "Function-এর ভেতরে name parameter ব্যবহার করো।", code: '    print(f"Hello, {name}!")' },
              { num: 3, text: "Function Call করো তিনবার।", code: 'greet("Alice")\ngreet("Bob")' },
              { num: 4, text: "Program চালাও।", code: "# Output: Hello, Alice!\\n# Output: Hello, Bob!" },
            ].map((step, i) => (
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-rose-800 via-pink-800 to-purple-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 6: D.R.Y. Concept</p>
                  <p className="text-white/50 text-xs mt-1">Learn about Code Reusability</p>
                </div>
              </div>
            </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={FADE_UP} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🏆</span>
              <p className="text-sm font-bold text-purple-800">Goal: D.R.Y. Function লেখো!</p>
            </div>
            ...
            {solutionOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="overflow-hidden">
                ...
                <p className="pl-4"><span className="text-green-400">print</span><span className="text-gray-300">(f"Hello, {"{name}"}!")</span></p>
                    ...
                    <p><span className="text-cyan-400">greet</span><span className="text-gray-300">(</span><span className="text-green-400">"Alice"</span><span className="text-gray-300">)</span></p>
                    <p><span className="text-cyan-400">greet</span><span className="text-gray-300">(</span><span className="text-green-400">"Bob"</span><span className="text-gray-300">)</span></p>
                    <p><span className="text-cyan-400">greet</span><span className="text-gray-300">(</span><span className="text-green-400">"Charlie"</span><span className="text-gray-300">)</span></p>
                  </div>
                </div>
              </motion.div>
            )}
        ...
        <div className="px-6 py-6">
          <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-2xl p-4 mb-4">
            <p className="text-sm text-indigo-800 leading-relaxed">
              D.R.Y. principle সম্পর্কে জিজ্ঞাসা করো! AI তোমাকে help করবে।
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <input ref={aiRef} value={aiInput} onChange={(e) => setAiInput(e.target.value)}
              ...
              placeholder="Ask about D.R.Y. principle..."
