...
const LESSON_ID = "ch6-calculator";
const SOLUTION_CODE = `def add(a, b):\n    return a + b\n\n\ndef subtract(a, b):\n    return a - b\n\n\ndef multiply(a, b):\n    return a * b\n\n\ndef divide(a, b):\n    if b == 0:\n        return "Error: Division by zero!"\n    return a / b\n\n\nprint(add(10, 5))\nprint(subtract(10, 5))\nprint(multiply(10, 5))\nprint(divide(10, 5))`;

const SUGGESTED_QUESTIONS = [
  "Calculator কিভাবে functions দিয়ে বানাই?",
  "Each operation-এর জন্য আলাদা function কেন?",
  "Function composition কী?",
  "Error handling কিভাবে করি?",
  "Modular code কেন ভালো?",
];

const MOTIVATIONAL = [
  "চমৎকার! Calculator তৈরি হয়েছে!",
  "দারুণ! তুমি পারবে!",
  "অসাধারণ চেষ্টা!",
  "তুমি সঠিক পথে আছো!",
  "বাহ! Calculator master!",
];

const HINTS = [
  "প্রতিটি operation-এর জন্য আলাদা function লিখো।",
  "add, subtract, multiply, divide functions তৈরি করো।",
  "return statement ব্যবহার করো।",
  "Functions কে call করে test করো।",
];
...
/* ── Calculator Operations Animation ── */
function CalculatorOperations() {
  const [step, setStep] = useState(-1);

  const steps = [
    { icon: "➕", label: "Add Function", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
    { icon: "➖", label: "Subtract Function", color: "text-red-400", bg: "bg-red-900/30 border-red-500/50" },
    { icon: "✖️", label: "Multiply Function", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "➗", label: "Divide Function", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
    { icon: "🧮", label: "Calculator Ready!", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
  ];
      ...
      <div className="space-y-3">
        {steps.map((s, i) => (
          <motion.div
            ...
            animate={i <= step ? { opacity: 1, x: 0 } : { opacity: 0.3, x: -20 }}
            transition={{ duration: 0.5, type: "spring" }}
            className={`flex items-center gap-3 p-3 rounded-xl border-2 ${i <= step ? s.bg : "bg-gray-800/30 border-gray-500/30"}`}
          >
            <span className="text-xl">{s.icon}</span>
            <p className={`text-sm font-bold ${i <= step ? s.color : "text-gray-600"}`}>{s.label}</p>
          </motion.div>
        ))}
      </div>
        ...
        {step >= 4 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              ...
              <p className="text-sm font-bold text-green-400">Calculator built with functions!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function InteractiveCalculator() {
  ...
  const calculate = useCallback(() => {
    ...
    if (isNaN(a) || isNaN(b)) { setResult("Error: Invalid numbers!"); return; }

    switch (operation) {
      case "add": setResult((a + b).toString()); break;
      case "subtract": setResult((a - b).toString()); break;
      case "multiply": setResult((a * b).toString()); break;
      case "divide":
        if (b === 0) { setResult("Error: Division by zero!"); }
        else { setResult((a / b).toString()); }
        break;
    }
  }, [num1, num2, operation]);
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-2">
        <input
          ...
          className="px-4 py-3 bg-gray-800 border border-gray-600 rounded-xl text-white font-mono text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400"
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          { op: "add", icon: "➕", color: "bg-green-500 hover:bg-green-600" },
          { op: "subtract", icon: "➖", color: "bg-red-500 hover:bg-red-600" },
          { op: "multiply", icon: "✖️", color: "bg-blue-500 hover:bg-blue-600" },
          { op: "divide", icon: "➗", color: "bg-purple-500 hover:bg-purple-600" },
        ].map((item) => (
          <button
            key={item.op}
            onClick={() => setOperation(item.op as typeof operation)}
            className={`px-4 py-3 rounded-xl text-white font-bold transition-all ${operation === item.op ? item.color + " ring-2 ring-white" : "bg-gray-700 hover:bg-gray-600"}`}
          >
            {item.icon}
          </button>
        ))}
      ...
      <motion.button
        whileTap={{ scale: 0.95 }}
        onClick={calculate}
        className="w-full px-4 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-bold rounded-xl transition-all shadow-md"
      >
        Calculate
      </motion.button>
      ...
      <AnimatePresence>
        {result !== null && (
          <motion.div
            ...
            className={`p-4 rounded-xl text-center font-mono text-lg font-bold ${result.includes("Error") ? "bg-red-900/30 border border-red-500/30 text-red-400" : "bg-green-900/30 border border-green-500/30 text-green-400"}`}
          >
            {result}
          </motion.div>
        )}
  ...
  const startDemo = useCallback(() => {
    ...
    const typeInterval = setInterval(() => {
      if (idx < fullCode.length) {
        ...
        setTimeout(() => {
          setPhase("running");
          setFlowSteps(["📞 Calling add(10, 5)..."]);
          setTimeout(() => {
            setFlowSteps(prev => [...prev, "⚡ Function Runs!"]);
            setTimeout(() => {
              setFlowSteps(prev => [...prev, "🔢 Returns: 15"]);
              setOutputLines(["15"]);
              setTimeout(() => {
                setFlowSteps(prev => [...prev, "📞 Calling subtract(10, 5)..."]);
                setTimeout(() => {
                  setFlowSteps(prev => [...prev, "⚡ Function Runs!"]);
                  setTimeout(() => {
                    setFlowSteps(prev => [...prev, "🔢 Returns: 5"]);
                    setOutputLines(prev => [...prev, "5"]);
                    setPhase("done");
                  }, 600);
                }, 800);
              }, 400);
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-gray-400 text-xs font-mono">calculator.py</span>
          </div>
        </div>
              ...
              <div>
                {line.trim().startsWith("def") ? (
                  <>
                    ...
                    <span className="text-cyan-400"> {line.trim().match(/def\s+(\w+)/)?.[1]}</span>
                    <span className="text-gray-300">({line.trim().match(/\((\w+),\s*(\w+)\)/)?.[1]}, {line.trim().match(/\((\w+),\s*(\w+)\)/)?.[2]}):</span>
                  </>
                ) : line.trim().startsWith("return") ? (
                  <span className="pl-4"><span className="text-purple-400">return</span><span className="text-green-400"> {line.trim().match(/return\s+(.+)/)?.[1]}</span></span>
                ) : line.trim().startsWith("print") ? (
                  <span className="pl-4"><span className="text-green-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">{line.trim().match(/print\((.+)\)/)?.[1]}</span><span className="text-gray-300">)</span></span>
                ) : (
                  <span>{line}</span>
                )}
              </div>
        ...
        {phase === "done" && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-bold text-green-700">Calculator built with modular functions!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export function Ch6Calculator({
  ...
  const handleRun = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      for (const line of lines) {
        ...
        if (defMatch) {
          if (currentFunc) {
            functions[currentFunc] = { params: currentParams, body: [...currentBody] };
          }
          currentFunc = defMatch[1];
          currentParams = defMatch[2].split(",").map(p => p.trim());
          currentBody = [];
          continue;
        }
      ...
      for (const line of lines) {
        const t = line.trim();
        if (t === "" || t.startsWith("#") || t.startsWith("def") || (line.startsWith("    ") || line.startsWith("\t"))) continue;

        const printMatch = t.match(/^print\((.+)\)$/);
        if (printMatch) {
          const arg = printMatch[1].trim();
          const funcCallMatch = arg.match(/^(\w+)\(([^)]+)\)$/);
          if (funcCallMatch) {
            const funcName = funcCallMatch[1];
            const funcArgs = funcCallMatch[2].split(",").map(a => parseFloat(a.trim()));
            if (functions[funcName] && funcArgs.every(a => !isNaN(a))) {
              const returnLine = functions[funcName].body.find(b => b.startsWith("return"));
              if (returnLine) {
                const expr = returnLine.replace("return", "").trim();
                if (expr.includes("+")) {
                  out.push((funcArgs[0] + funcArgs[1]).toString());
                  ...
                  if (funcArgs[1] === 0) {
                    out.push("Error: Division by zero!");
                  } else {
                    out.push((funcArgs[0] / funcArgs[1]).toString());
                  }
                }
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      let a = "";
      if (q.includes("Calculator") || q.includes("calculator")) { a = "Calculator তৈরি করতে প্রতিটি operation-এর জন্য আলাদা function লিখো — add, subtract, multiply, divide!"; }
      else if (q.includes("operation") || q.includes("Operation")) { a = "প্রতিটি operation-এর জন্য আলাদা function লিখলে code modular ও reusable হয়!"; }
      else if (q.includes("composition") || q.includes("compose")) { a = "Function composition মানে একটি function-এর output অন্য function-এ input হিসেবে ব্যবহার করা!"; }
      else if (q.includes("error") || q.includes("Error")) { a = "Error handling করতে if/else ব্যবহার করো — যেমন division by zero check!"; }
      else { a = "ভালো প্রশ্ন! Calculator তৈরি করতে modular functions লিখো — প্রতিটি operation আলাদা function!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        <motion.div variants={FADE_UP} className="px-6 py-5 bg-gradient-to-r from-red-500 via-pink-500 to-rose-600">
          <div className="flex items-center gap-3"><span className="text-3xl">📘</span><h2 className="text-xl font-extrabold text-white tracking-tight">Calculator</h2></div>
        </motion.div>
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">Calculator Functions</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              <span className="font-bold text-cyan-600">Calculator</span> তৈরি করতে আমরা <span className="font-bold text-purple-600">modular functions</span> ব্যবহার করি —
              প্রতিটি operation-এর জন্য আলাদা function!
            </p>
            <p className="text-gray-700 text-[15px] leading-relaxed mt-2">
              <span className="font-bold text-amber-600">add, subtract, multiply, divide</span> —
              সব function আলাদা আলাদা, কিন্তু একসাথে কাজ করে!
            </p>
          </motion.div>

          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🧮 Calculator Operations</p>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🔢 Interactive Calculator</p>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> Calculator তৈরি করতে প্রতিটি operation-এর জন্য আলাদা function লিখো।
                Modular code সবসময় ভালো!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">Calculator functions আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "🏦", label: "Bank", desc: "Deposit, Withdraw, Transfer — সব function!", code: "bank_operation()" },
              { icon: "🎮", label: "Game", desc: "Move, Jump, Attack — সব function!", code: "game_action()" },
              { icon: "📱", label: "Phone", desc: "Call, Message, Email — সব function!", code: "phone_action()" },
              { icon: "🚗", label: "Car", desc: "Start, Stop, Accelerate — সব function!", code: "car_action()" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🏦 Bank Operations Example</p>
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
                    className="px-4 py-3 bg-red-900/30 border border-red-500/30 rounded-xl">
                    <p className="text-red-400 font-mono text-sm">Mixed code</p>
                  </motion.div>
                  <p className="text-xs text-gray-500 mt-2">❌ Messy!</p>
                </div>
                ...
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}
                    ...
                    <p className="text-green-400 font-mono text-sm">Modular functions</p>
                  </motion.div>
                  <p className="text-xs text-gray-500 mt-2">✅ Clean!</p>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> Calculator তৈরি করতে modular functions লিখো —
                প্রতিটি operation আলাদা function, কিন্তু একসাথে কাজ করে!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: Calculator Functions!</p>
                  <p className="text-xs text-gray-500 mt-1">calculator.py ফাইলে calculator functions লেখো</p>
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-purple-400">def</span> <span className="text-cyan-400">add</span><span className="text-gray-300">(a, b):</span></p>
                <p className="pl-4"><span className="text-purple-400">return</span> <span className="text-green-400">a + b</span></p>
                <p></p>
                <p><span className="text-purple-400">def</span> <span className="text-cyan-400">subtract</span><span className="text-gray-300">(a, b):</span></p>
                <p className="pl-4"><span className="text-purple-400">return</span> <span className="text-green-400">a - b</span></p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "add function লিখো।", code: "def add(a, b):" },
              { num: 2, text: "subtract function লিখো।", code: "def subtract(a, b):" },
              { num: 3, text: "multiply আর divide functions লিখো।", code: "def multiply(a, b):\ndef divide(a, b):" },
              { num: 4, text: "Functions কে call করে test করো।", code: "print(add(10, 5))\nprint(subtract(10, 5))" },
            ].map((step, i) => (
              <motion.div key={i} variants={FADE_UP} onClick={() => toggleStep(i)}
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 bg-gray-900 rounded-lg p-2.5 font-mono text-xs">
                    <span className={step.code.includes("#") ? "text-gray-400" : step.code.includes("return") ? "text-purple-400" : step.code.includes("def") ? "text-purple-400" : "text-cyan-400"}>{step.code}</span>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={FADE_UP} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-rose-800 via-pink-800 to-purple-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 6: Calculator</p>
                  <p className="text-white/50 text-xs mt-1">Build Calculator with Functions</p>
                </div>
              </div>
            </div>
            <div className="absolute top-3 right-3 px-2.5 py-1 bg-black/70 backdrop-blur-sm rounded-lg text-white text-xs font-mono">9:00</div>
            <AnimatePresence>
              {videoComplete && (
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute top-3 left-3 px-3 py-1 bg-green-500 rounded-full text-white text-xs font-bold flex items-center gap-1.5 shadow-lg">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>সম্পূর্ণ
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-gray-500 w-10 text-right">9:00</span>
          </div>
        </div>
      </motion.section>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🏆</span>
              <p className="text-sm font-bold text-purple-800">Goal: Calculator Functions!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-purple-400">def add(a, b):</p>
              <p className="text-purple-400 pl-4">return a + b</p>
              <p></p>
              <p className="text-cyan-400">print(add(10, 5))</p>
            </div>
          </motion.div>
        ...
        <div className="px-6 py-5">
          <button onClick={() => setSolutionOpen(!solutionOpen)} className="w-full flex items-center justify-between px-5 py-4 bg-amber-50 hover:bg-amber-100 border-2 border-amber-200 rounded-2xl transition-all duration-200">
            <span className="flex items-center gap-2.5 text-sm font-bold text-amber-800">
              <span className="text-xl">{solutionOpen ? "👁️" : "🔒"}</span>{solutionOpen ? "সমাধান লুকান" : "সমাধান দেখান"}
            </span>
              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="overflow-hidden">
                <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-md">
                  ...
                  <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
                    ...
                    <p className="pl-4"><span className="text-purple-400">return</span> <span className="text-green-400">a - b</span></p>
                    <p></p>
                    <p><span className="text-purple-400">def</span> <span className="text-cyan-400">multiply</span><span className="text-gray-300">(a, b):</span></p>
                    <p className="pl-4"><span className="text-purple-400">return</span> <span className="text-green-400">a * b</span></p>
                    <p></p>
                    <p><span className="text-purple-400">def</span> <span className="text-cyan-400">divide</span><span className="text-gray-300">(a, b):</span></p>
                    <p className="pl-4"><span className="text-purple-400">if</span> <span className="text-green-400">b == 0:</span></p>
                    <p className="pl-8"><span className="text-purple-400">return</span> <span className="text-green-400">"Error: Division by zero!"</span></p>
                    <p className="pl-4"><span className="text-purple-400">return</span> <span className="text-green-400">a / b</span></p>
                    <p></p>
                    <p><span className="text-green-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">add</span><span className="text-gray-300">(</span><span className="text-green-400">10, 5</span><span className="text-gray-300">))</span></p>
                    <p><span className="text-green-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">subtract</span><span className="text-gray-300">(</span><span className="text-green-400">10, 5</span><span className="text-gray-300">))</span></p>
                    <p><span className="text-green-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">multiply</span><span className="text-gray-300">(</span><span className="text-green-400">10, 5</span><span className="text-gray-300">))</span></p>
                    <p><span className="text-green-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">divide</span><span className="text-gray-300">(</span><span className="text-green-400">10, 5</span><span className="text-gray-300">))</span></p>
                  </div>
                </div>
              </motion.div>
            )}
        ...
        <div className="px-6 py-6">
          <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-2xl p-4 mb-4">
            <p className="text-sm text-indigo-800 leading-relaxed">
              Calculator functions সম্পর্কে জিজ্ঞাসা করো! AI তোমাকে help করবে।
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <input ref={aiRef} value={aiInput} onChange={(e) => setAiInput(e.target.value)}
              ...
              placeholder="Ask about Calculator functions..."
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={FADE_UP} className="flex items-center justify-between pt-4">
        <button onClick={onComplete.bind(null, LESSON_ID)}
          ...
          {isCompleted ? "✓ সম্পূর্ণ" : "সম্পূর্ণ চিহ্নিত করুন"}
        </button>
        {hasNext && (
