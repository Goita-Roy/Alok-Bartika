...
const LESSON_ID = "ch8-basic";
const SOLUTION_CODE = "import math\n\nprint(math.sqrt(36))";
...
const SUGGESTED_QUESTIONS = [
  ...
  "`math` Module কী?",
  "`sqrt()` কী কাজ করে?",
  "Module কেন ব্যবহার করি?",
];

const MOTIVATIONAL = [
  "চমৎকার! Module import হয়েছে!",
  "দারুণ! তুমি পারবে!",
  "অসাধারণ চেষ্টা!",
  "তুমি সঠিক পথে আছো!",
  "বাহ! Module master!",
...
const HINTS = [
  ...
  "`math` Module import করো।",
  "`sqrt()` function ব্যবহার করো।",
  "Returned value print করো।",
];
...
/* ── Module Blueprint Animation ── */
function ModuleBlueprintAnimation() {
  const [step, setStep] = useState(-1);

  const steps = [
    { icon: "📦", label: "Python Module", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "📂", label: "Import Module", color: "text-cyan-400", bg: "bg-cyan-900/30 border-cyan-500/50" },
    { icon: "⚡", label: "Use Functions", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "✅", label: "Save Time!", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
  ];
        ...
        {step >= 3 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              ...
              <p className="text-sm font-bold text-green-400">Module import করে functions ব্যবহার করো — সময় বাঁচাও!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function LearningFlowAnimation() {
  ...
  const steps = [
    { icon: "✍️", label: "Write Code Once", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "💾", label: "Save as Module", color: "text-cyan-400", bg: "bg-cyan-900/30 border-cyan-500/50" },
    { icon: "📥", label: "Import Module", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "🔄", label: "Reuse Anywhere", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
    { icon: "🚀", label: "Easy Programming!", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
  ];
      ...
      <AnimatePresence>
        {step >= 4 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              <span className="text-lg">🎉</span>
              <p className="text-sm font-bold text-green-400">Module লিখো একবার, ব্যবহার করো অনেক জায়গায়!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function ImportKeywordIntro() {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {show ? "🔄 পুনরায় দেখুন" : "▶ import Keyword দেখান"}
      </button>
      ...
      <div className="flex items-center justify-center gap-3">
        ...
        >
          <div className="flex gap-2">
            ...
            >
              <span className="text-lg font-mono font-bold text-white">math</span>
            </motion.div>
          ...
          >
            import keyword + module name + colon
          </motion.p>
        </motion.div>
        ...
        >
          <span className="text-xl font-mono font-bold text-white">sqrt</span>
        </motion.div>
      </div>
        ...
        {show && (
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-purple-900/30 border border-purple-500/30 rounded-xl">
              <span className="text-lg">💡</span>
              <p className="text-sm font-bold text-purple-400">Modules use `import` keyword + module name + functions!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
  ...
  const startDemo = useCallback(() => {
    ...
    const typeInterval = setInterval(() => {
      if (idx < fullCode.length) {
        ...
        setTimeout(() => {
          setPhase("running");
          setFlowSteps(["📦 Importing Module..."]);
          setTimeout(() => {
            setFlowSteps(prev => [...prev, "🧮 Using sqrt()..."]);
            setTimeout(() => {
              setFlowSteps(prev => [...prev, "✅ Result Found!"]);
              setOutputLines(["5.0"]);
              setPhase("done");
            }, 600);
          }, 800);
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-gray-400 text-xs font-mono">modules.py</span>
          </div>
        </div>
          ...
          {codeLines.map((line, i) => (
            <div key={i} className="flex">
              ...
              <div>
                ...
                ) : line.trim().startsWith("print(math.sqrt") ? (
                  <span className="pl-4"><span className="text-green-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">math.sqrt</span><span className="text-gray-300">(</span><span className="text-amber-400">25</span><span className="text-gray-300">))</span></span>
                ) : (
                  <span>{line}</span>
                )}
              </div>
        ...
        <div className="px-4 pb-3 bg-gray-900">
          <div className="space-y-1.5">
            ...
            {flowSteps.length === 0 && (
              <p className="text-[10px] text-gray-500 font-mono">📦 Module import করে functions ব্যবহার করো...</p>
            )}
          </div>
        </div>
        ...
        {phase === "done" && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-bold text-green-700">`import math` দিয়ে `math.sqrt(25)` ব্যবহার করে `25`-এর square root বের হলো — <span className="text-lg">5.0</span>!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export function Ch8Basic({
  ...
  const handleRun = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      for (const line of lines) {
        ...
        if (t === "import math") continue;

        const sqrtMatch = t.match(/^print\s*\(\s*math\.sqrt\s*\(\s*(-?\d+\.?\d*)\s*\)\s*\)$/);
        if (sqrtMatch) {
          const num = parseFloat(sqrtMatch[1]);
          if (num < 0) {
            out.push("❌ math domain error: negative number এর sqrt নেই!");
          } else {
            out.push(String(Math.sqrt(num)));
          }
          continue;
        }

        if (t.startsWith("print")) {
          const innerMatch = t.match(/^print\s*\(\s*(.+)\s*\)$/);
          if (innerMatch) {
            const expr = innerMatch[1].trim();
            const sqrtInline = expr.match(/^math\.sqrt\s*\(\s*(-?\d+\.?\d*)\s*\)$/);
            if (sqrtInline) {
              const num = parseFloat(sqrtInline[1]);
              if (num < 0) {
                out.push("❌ math domain error: negative number এর sqrt নেই!");
              } else {
                out.push(String(Math.sqrt(num)));
              ...
              out.push("⚠️ এই expression বোঝা যায়নি।");
            }
          }
        }
      }

      if (out.length === 0) out.push("⚠️ কোনো output পাওয়া যায়নি। `import math` এবং `print(math.sqrt(N))` লিখো।");
      setConsoleLines(out);
      setIsRunning(false);
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      let a = "";
      if (q.includes("Module") || q.includes("module")) { a = "Module হলো Python-এ একটি reusable code file যেটা অন্য কোডে import করে ব্যবহার করা যায়! যেমন: `math`, `random`, `os`।"; }
      else if (q.includes("import")) { a = "`import` keyword দিয়ে আমরা Module import করি। যেমন: `import math` — এতে math module-এর সব functions ব্যবহার করা যায়!"; }
      else if (q.includes("math")) { a = "`math` Module হলো Python-এর built-in module যেটাতে mathematical functions আছে — যেমন `sqrt()`, `pow()`, `ceil()`, `floor()`!"; }
      else if (q.includes("sqrt")) { a = "`math.sqrt()` function একটি সংখ্যার square root বের করে। যেমন: `math.sqrt(36)` = `6.0`, `math.sqrt(25)` = `5.0`!"; }
      else if (q.includes("কেন") || q.includes("ব্যবহার")) { a = "Module ব্যবহার করলে আমাদের নিজে সব function লিখতে হয় না — অন্যরা লিখে রাখা functions reuse করতে পারি!"; }
      else { a = "ভালো প্রশ্ন! Python Module হলো reusable code — `import` করে `math.sqrt()` এর মতো functions সহজে ব্যবহার করো!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">Python Module কী?</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              <span className="font-bold text-cyan-600">Python Module</span> হলো একটি <span className="font-bold text-purple-600">reusable code file</span> — যেটাতে ready-made functions থাকে যেগুলো আমরা সরাসরি ব্যবহার করতে পারি!
            </p>
            <p className="text-gray-700 text-[15px] leading-relaxed mt-2">
              <span className="font-bold text-amber-600">`import` keyword</span> দিয়ে Module import করি —
              তারপন <span className="font-bold text-green-600">`math.sqrt()`</span> এর মতো functions ব্যবহার করি!
            </p>
          </motion.div>

          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">📦 Python Module → Import → Use → Save Time!</p>
              <ModuleBlueprintAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🔍 The `import` Keyword</p>
              <ImportKeywordIntro />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> Module
                হলো ready-made code —
                `import` করে সহজে functions ব্যবহার করো!
                `math.sqrt()` দিয়ে square root বের করো।
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">Python Module আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "🧰", label: "Toolbox", desc: "একটি বাক্সে সব হাতিয়ার!", code: "import toolbox" },
              { icon: "📚", label: "Library", desc: "অনেক বই এক জায়গায়!", code: "import library" },
              { icon: "🍽️", label: "Kitchen Drawer", desc: "সব রান্নার সামান এক জায়গায়!", code: "import kitchen" },
              { icon: "🧱", label: "LEGO Box", desc: "বিভিন্ন পিস একসাথে!", code: "import lego" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🧰 Module Example</p>
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
                    className="px-4 py-3 bg-red-900/30 border border-red-500/30 rounded-xl">
                    <p className="text-red-400 font-mono text-sm">নিজেই সব function লেখো</p>
                  </motion.div>
                ...
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}
                    ...
                    <p className="text-green-400 font-mono text-sm">Module import করো!</p>
                  </motion.div>
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                ...
                Module ঠিক যেমন Toolbox —
                ready-made functions এক জায়গায়, import করে ব্যবহার করো!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: math Module দিয়ে sqrt বের করো!</p>
                  <p className="text-xs text-gray-500 mt-1">modules.py ফাইলে import এবং sqrt() ব্যবহার করো</p>
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-purple-400">import</span> <span className="text-cyan-400">math</span></p>
                <p></p>
                <p><span className="text-green-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">math.sqrt</span><span className="text-gray-300">(</span><span className="text-amber-400">36</span><span className="text-gray-300">))</span></p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "`import math` দিয়ে math Module import করো।", code: "import math" },
              { num: 2, text: "`math.sqrt(36)` দিয়ে 36-এর square root বের করো।", code: "math.sqrt(36)" },
              { num: 3, text: "Result কে `print()` দিয়ে output করো।", code: "print(math.sqrt(36))" },
              { num: 4, text: "Program চালাও এবং output দেখো।", code: "python modules.py" },
            ].map((step, i) => (
              <motion.div key={i} variants={FADE_UP} onClick={() => toggleStep(i)}
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 bg-gray-900 rounded-lg p-2.5 font-mono text-xs">
                    <span className={step.code.includes("import") ? "text-purple-400" : step.code.includes("sqrt") ? "text-cyan-400" : step.code.includes("print") ? "text-green-400" : "text-gray-400"}>{step.code}</span>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={FADE_UP} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-rose-800 via-pink-800 to-purple-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 8: Python Modules - Basic</p>
                  <p className="text-white/50 text-xs mt-1">Learn about Python Modules &amp; Import</p>
                </div>
              </div>
            </div>
            <div className="absolute top-3 right-3 px-2.5 py-1 bg-black/70 backdrop-blur-sm rounded-lg text-white text-xs font-mono">5:30</div>
            <AnimatePresence>
              {videoComplete && (
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-gray-500 w-10 text-right">5:30</span>
          </div>
        </div>
      </motion.section>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🏆</span>
              <p className="text-sm font-bold text-purple-800">Goal: math Module দিয়ে sqrt বের করো!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-purple-400">import math</p>
              <p></p>
              <p className="text-green-400">print(math.sqrt(36))</p>
            </div>
          </motion.div>
        ...
        <div className="px-6 py-6">
          <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-2xl p-4 mb-4">
            <p className="text-sm text-indigo-800 leading-relaxed">
              Python Module সম্পর্কে জিজ্ঞাসা করো! AI তোমাকে help করবে।
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <input ref={aiRef} value={aiInput} onChange={(e) => setAiInput(e.target.value)}
              ...
              placeholder="Ask about Python Modules..."
