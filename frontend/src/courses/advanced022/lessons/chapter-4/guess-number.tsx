...
const LESSON_ID = "ch4-guess-number";
const SOLUTION_CODE = `secret = 7\nguess = 0\n\nwhile guess != secret:\n    guess = int(input("Guess the number: "))\n\nprint("🎉 Correct!")`;

const SUGGESTED_QUESTIONS = [
  "while Loop কী?",
  "secret Variable কী?",
  "guess Variable কেন ব্যবহার করছি?",
  "int() কেন ব্যবহার করি?",
  "Loop কখন বন্ধ হবে?",
];
...
const HINTS = [
  "একটি Variable তৈরি করো: secret = 7",
  "while loop ব্যবহার করো: while guess != secret:",
  "!= দিয়ে compare করো — guess আর secret সমান কি না!",
  "input() দিয়ে guess নাও এবং int() দিয়ে number-এ রূপান্তর করো।",
];
...
/* ── Guess Number Learning Flow Animation ── */
function GuessFlowAnimation() {
...
function GuessNumberGame() {
  ...
  const [phase, setPhase] = useState<"input" | "checking" | "won">("input");
  const [lastResult, setLastResult] = useState<"low" | "high" | null>(null);
  ...
  const handleSubmit = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (num === secret) {
        ...
        setLastResult(num < secret ? "low" : "high");
        setCurrentGuess("");
        setPhase("input");
      }
    }, 800);
      ...
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
        ...
        <div className="bg-gray-950 rounded-xl p-4 border border-gray-700">
          <div className="text-center mb-4">
            <motion.div animate={phase === "won" ? { scale: [1, 1.2, 1] } : {}} transition={{ duration: 0.5, repeat: phase === "won" ? 3 : 0 }}
            ...
            <div className="flex flex-wrap justify-center gap-2 mb-4">
              {guesses.map((g, i) => (
                <motion.div key={i} initial={{ scale: 0 }} animate={{ scale: 1 }}
                  className={`w-10 h-10 rounded-lg flex items-center justify-center text-sm font-bold ${g === secret ? "bg-green-500 text-white" : "bg-gray-700 text-gray-300"}`}>
                  {g}
                </motion.div>
              ))}
            </div>
          ...
          <AnimatePresence>
            {lastResult && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                className={`text-center mb-3 text-sm font-bold ${lastResult === "low" ? "text-cyan-400" : "text-red-400"}`}>
                {lastResult === "low" ? "📈 Too low! Go higher." : "📉 Too high! Go lower."}
              </motion.div>
            )}
          </AnimatePresence>
...
function ExampleSection() {
  ...
  const startDemo = useCallback(() => {
    ...
    const t = setInterval(() => {
      ...
      setOutputLines(prev => [...prev, `Guess the number: ${guesses[attempt]}`]);
      
      setTimeout(() => {
        if (guesses[attempt] !== 7) {
          setOutputLines(prev => [...prev, ""]);
        } else {
          setOutputLines(prev => [...prev, "", "🎉 Correct!"]);
          clearInterval(t);
      ...
      attempt++;
      if (attempt >= guesses.length) clearInterval(t);
    }, 1200);
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-gray-400 text-xs font-mono">guess_number.py</span>
          </div>
        </div>
        <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
          <div className="flex">
            ...
            <div><span className="text-cyan-400">secret</span><span className="text-yellow-400"> = </span><span className="text-amber-400">7</span></div>
          </div>
          <div className="flex">
            <span className="text-gray-600 text-xs w-6 text-right mr-3 select-none">2</span>
            <div><span className="text-cyan-400">guess</span><span className="text-yellow-400"> = </span><span className="text-amber-400">0</span></div>
          </div>
          <div className="flex">
          ...
          <div className="flex">
            ...
            <div><span className="text-purple-400">while</span><span className="text-cyan-400"> guess</span><span className="text-red-400"> != </span><span className="text-cyan-400">secret</span><span className="text-gray-300">:</span></div>
          </div>
          <div className="flex">
            <span className="text-gray-600 text-xs w-6 text-right mr-3 select-none">5</span>
            <div className="pl-4"><span className="text-cyan-400">guess</span><span className="text-yellow-400"> = </span><span className="text-amber-400">int</span><span className="text-gray-300">(</span><span className="text-amber-400">input</span><span className="text-gray-300">(</span><span className="text-green-400">"Guess the number: "</span><span className="text-gray-300">))</span></div>
          </div>
          <div className="flex">
          ...
          <div className="flex">
            <span className="text-gray-600 text-xs w-6 text-right mr-3 select-none">7</span>
            <div><span className="text-purple-400">print</span><span className="text-gray-300">(</span><span className="text-green-400">"🎉 Correct!"</span><span className="text-gray-300">)</span></div>
          </div>
        </div>
        ...
        <div className="px-4 pb-3 bg-gray-900">
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-gray-500 font-bold">Guess:</span>
            <div className="flex gap-1.5">
              {[1, 2, 3].map(n => (
                <motion.div key={n}
                  animate={n <= currentAttempt ? { scale: [1, 1.3, 1], opacity: 1 } : { opacity: 0.3 }}
                  className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold ${n <= currentAttempt ? (n === 3 ? "bg-green-500 text-white" : "bg-purple-500 text-white") : "bg-gray-700 text-gray-500"}`}>
                  {n}
                </motion.div>
              ))}
            </div>
            ...
            {outputLines.length === 0 ? (
              ...
              outputLines.map((line, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                  className={line === "🎉 Correct!" ? "text-green-400 font-bold" : line.startsWith("Guess the number:") ? "text-cyan-400" : "text-gray-500"}>
                  {line}
                </motion.div>
              ))
            )}
...
export function Ch4GuessNumber({
  ...
  const handleRun = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      for (const line of lines) {
        ...
        const secretMatch = t.match(/^secret\s*=\s*(\d+)$/);
        if (secretMatch) { secretVal = parseInt(secretMatch[1]); continue; }

        const guessMatch = t.match(/^guess\s*=\s*\d+$/);
        ...
        if (inWhileLoop) {
          if (line.startsWith("    ") || line.startsWith("\t")) {
            ...
            out.push(`Guess the number: ${secretVal}`);
            out.push("", "🎉 Correct!");
            inWhileLoop = false;
            loopBody = [];
          }
      ...
      if (hasSuccess && !hasError) {
        ...
        setStepsDone([true, true, true, true, true, true]);
      }
    }, 650);
  }, [editorCode]);
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      let a = "";
      if (q.includes("while Loop") || q.includes("while loop")) { a = "while Loop হলো Python-এ এমন loop যেটা একটি শর্ত (condition) False না হওয়া পর্যন্ত চলতে থাকে। Guess Number-এ loop চলে guess != secret শর্তে!"; }
      else if (q.includes("secret")) { a = "secret Variable-এ সংখ্যাটি সংরক্ষিত থাকে যেটি খেলোয়াড়কে অনুমান করতে হবে। এটি একটি constant মান — পরিবর্তন হয় না!"; }
      else if (q.includes("guess")) { a = "guess Variable-এ ব্যবহারকারীর অনুমান সংরক্ষিত থাকে। প্রতিবার loop চলাকালীন নতুন guess দেওয়া হয়। loop শুধু guess == secret হলে বন্ধ হয়!"; }
      else if (q.includes("int()") || q.includes("int")) { a = "int() function string কে integer number-এ রূপান্তর করে। input() সবসময় string দেয়, তাই int() দিয়ে number-এ রূপান্তর করতে হয়!"; }
      else if (q.includes("বন্ধ") || q.includes("stop")) { a = "Loop বন্ধ হয় যখন guess == secret হয়। তখন while loop-এর শর্ত guess != secret False হয় — loop বন্ধ!"; }
      else { a = "ভালো প্রশ্ন! Guess Number একটি মজার খেলা যেখানে while Loop ব্যবহার করা হয়। নিচের suggested questions দেখো!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">Guess the Number কী?</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              <span className="font-bold text-cyan-600">while Loop</span> দিয়ে একটি মজার <span className="font-bold text-purple-600">Guess the Number</span> খেলা তৈরি করা যায়!
            </p>
            <p className="text-gray-700 text-[15px] leading-relaxed mt-2">
              Python <span className="font-bold text-red-600">গোপনীয়ভাবে</span> একটি সংখ্যা বাছাই করে। তুমি বারবার অনুমান করো —
              সঠিক উত্তর না পাওয়া পর্যন্ত loop চলে!
            </p>
          </motion.div>

          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🎲 Guess Number Flow</p>
              <GuessFlowAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🎮 Try the Game!</p>
              <GuessNumberGame />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> Guess Number
                while Loop-এর একটি চমৎকার উদাহরণ —
                সঠিক উত্তর না পাওয়া পর্যন্ত খেলা চলে!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">Guess Number আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "🎁", label: "Treasure Hunt", desc: "খোঁজো গোপন সম্পদ!", code: "Find it!" },
              { icon: "🎲", label: "Dice Game", desc: "পাশা ফেলো আর অনুমান করো", code: "Roll & guess" },
              { icon: "🎯", label: "Lucky Number", desc: "ভাগ্য সংখ্যা খুঁজো", code: "Lucky?" },
              { icon: "🎮", label: "Mobile Game", desc: "ফোনে খেলো!", code: "Play & win" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🎁 Treasure Hunt Example</p>
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                ...
                ঠিক যেমন বন্ধু একটি সংখ্যা ভাবে আর তুমি অনুমান করো —
                while Loop-ও সঠিক উত্তর না পাওয়া পর্যন্ত চলে!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: Guess the Number তৈরি করো!</p>
                  <p className="text-xs text-gray-500 mt-1">guess_number.py ফাইলে while loop দিয়ে খেলা তৈরি করো</p>
                  <div className="flex items-center gap-2 mt-2">
                    {stepsDone.map((d, i) => (<div key={i} className={`h-2 rounded-full transition-all duration-500 ${i === 0 ? "w-16" : i === 1 ? "w-12" : i === 2 ? "w-12" : i === 3 ? "w-10" : i === 4 ? "w-10" : "w-8"} ${d ? "bg-green-400" : "bg-orange-200"}`} />))}
                    <span className="text-xs font-bold text-gray-500 ml-1">{stepsDone.filter(Boolean).length}/6</span>
                  </div>
                </div>
              </div>
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-cyan-400">secret</span> <span className="text-yellow-400">=</span> <span className="text-amber-400">7</span></p>
                <p><span className="text-cyan-400">guess</span> <span className="text-yellow-400">=</span> <span className="text-amber-400">0</span></p>
                <p></p>
                <p><span className="text-purple-400">while</span> <span className="text-cyan-400">guess</span> <span className="text-red-400">!=</span> <span className="text-cyan-400">secret</span><span className="text-gray-300">:</span></p>
                <p className="pl-4"><span className="text-cyan-400">guess</span> <span className="text-yellow-400">=</span> <span className="text-amber-400">int</span><span className="text-gray-300">(</span><span className="text-amber-400">input</span><span className="text-gray-300">(</span><span className="text-green-400">"Guess the number: "</span><span className="text-gray-300">))</span></p>
                <p></p>
                <p><span className="text-purple-400">print</span><span className="text-gray-300">(</span><span className="text-green-400">"🎉 Correct!"</span><span className="text-gray-300">)</span></p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "secret Variable তৈরি করো।", code: "secret = 7" },
              { num: 2, text: "guess Variable তৈরি করো।", code: "guess = 0" },
              { num: 3, text: "while Loop ব্যবহার করো।", code: "while guess != secret:" },
              { num: 4, text: "ব্যবহারকারীর কাছ থেকে guess নাও।", code: '    guess = int(input("Guess: "))' },
              { num: 5, text: "guess আর secret compare করো।", code: "# guess != secret → loop runs" },
              { num: 6, text: "সঠিক উত্তর পেলে Correct! প্রিন্ট করো।", code: 'print("🎉 Correct!")' },
            ].map((step, i) => (
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={FADE_UP} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-rose-800 via-pink-800 to-purple-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 4: Guess Number</p>
                  <p className="text-white/50 text-xs mt-1">Build a number guessing game</p>
                </div>
              </div>
            </div>
            <div className="absolute top-3 right-3 px-2.5 py-1 bg-black/70 backdrop-blur-sm rounded-lg text-white text-xs font-mono">6:20</div>
            <AnimatePresence>
              {videoComplete && (
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-gray-500 w-10 text-right">6:20</span>
          </div>
        </div>
      </motion.section>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🏆</span>
              <p className="text-sm font-bold text-purple-800">Goal: Guess the Number game তৈরি করো!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-cyan-400">secret = 7 <span className="text-gray-500"># Secret number</span></p>
              <p className="text-cyan-400">guess = 0 <span className="text-gray-500"># Start guess</span></p>
              <p className="text-purple-400">while guess != secret:</p>
              <p className="text-green-400 pl-4">guess = int(input("Guess: "))</p>
            </div>
          </motion.div>
            ...
            {solutionOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="overflow-hidden">
                <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-md">
                  ...
                  <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
                    {[
                      { parts: [{ c: "text-cyan-400", t: "secret" }, { c: "text-yellow-400", t: " = " }, { c: "text-amber-400", t: "7" }] },
                      { parts: [{ c: "text-cyan-400", t: "guess" }, { c: "text-yellow-400", t: " = " }, { c: "text-amber-400", t: "0" }] },
                      { parts: [] },
                      { parts: [{ c: "text-purple-400", t: "while" }, { c: "text-cyan-400", t: " guess" }, { c: "text-red-400", t: " != " }, { c: "text-cyan-400", t: "secret" }, { c: "text-gray-300", t: ":" }] },
                      { parts: [{ c: "text-cyan-400", t: "    guess" }, { c: "text-yellow-400", t: " = " }, { c: "text-amber-400", t: "int" }, { c: "text-gray-300", t: "(" }, { c: "text-amber-400", t: "input" }, { c: "text-gray-300", t: "(" }, { c: "text-green-400", t: '"Guess the number: "' }, { c: "text-gray-300", t: "))" }] },
                      { parts: [] },
                      { parts: [{ c: "text-purple-400", t: "print" }, { c: "text-gray-300", t: "(" }, { c: "text-green-400", t: '"🎉 Correct!"' }, { c: "text-gray-300", t: ")" }] },
                    ].map((item, i) => (
            ...
            <div className="h-72 overflow-y-auto p-4 space-y-3">
              {aiMessages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-xs text-gray-500 max-w-[240px] mb-4 leading-relaxed">Guess Number সম্পর্কে যেকোনো প্রশ্ন করো! নিচের যেকোনো প্রশ্নে ক্লিক করো।</p>
