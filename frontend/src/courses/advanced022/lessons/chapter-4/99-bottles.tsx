...
const LESSON_ID = "ch4-99-bottles";
const SOLUTION_CODE = `for bottles in range(99, 0, -1):\n    print(bottles, "bottles")`;

const SUGGESTED_QUESTIONS = [
  "Countdown Loop কী?",
  "range(99, 0, -1) কীভাবে কাজ করে?",
  "-1 কেন ব্যবহার করছি?",
  "Loop কখন বন্ধ হবে?",
  "for Loop দিয়ে উল্টোভাবে গণনা কীভাবে করি?",
];
...
const HINTS = [
  "for Loop ব্যবহার করো: for bottles in range(99, 0, -1):",
  "range(99, 0, -1) দিয়ে ৯৯ থেকে ১ পর্যন্ত গণনা করো।",
  "-1 মানে loop উল্টোদিকে চলবে — প্রতিবার ১ করে কমবে!",
  "print() দিয়ে প্রতিবার bottles সংখ্যা প্রিন্ট করো।",
];
...
/* ── Countdown Flow Animation ── */
function CountdownFlowAnimation() {
  const [step, setStep] = useState(-1);

  const steps = [
    { icon: "🧃", label: "৯৯ বোতল", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "🧃", label: "৯৮ বোতল", color: "text-cyan-400", bg: "bg-cyan-900/30 border-cyan-500/50" },
    { icon: "🧃", label: "৯৭ বোতল", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "⬇️", label: "...", color: "text-gray-400", bg: "bg-gray-800/30 border-gray-500/50" },
    { icon: "🧃", label: "১ বোতল", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
    { icon: "🎉", label: "০ বোতল!", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
  ];
        ...
        {step >= 5 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              ...
              <p className="text-sm font-bold text-green-400">Countdown complete!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
/* ── Bottle Shelf Animation ── */
function BottleShelfAnimation() {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
        <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🧃 Bottle Shelf</p>
        
        {/* Shelf */}
        <div className="bg-gray-950 rounded-xl p-4 border border-gray-700">
          <div className="flex items-end justify-center gap-1 min-h-[120px] mb-4">
            {Array.from({ length: 12 }, (_, i) => (
              <motion.div key={i}
                animate={i < count ? { opacity: 1, y: 0, scale: 1 } : { opacity: 0, y: -20, scale: 0.5 }}
          ...
          <div className="text-center">
            <motion.div key={count} initial={{ scale: 1.3 }} animate={{ scale: 1 }}
              className="inline-flex items-center gap-2 px-4 py-2 bg-cyan-900/30 border border-cyan-500/30 rounded-xl">
              <span className="text-3xl font-bold text-cyan-400">{count}</span>
              <span className="text-sm text-cyan-300">বাকি {count === 1 ? "১টি বোতল" : `${count}টি বোতল`}</span>
            </motion.div>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <button onClick={startCountdown}
          className={`flex-1 px-4 py-2 font-bold rounded-xl transition-colors ${isRunning ? "bg-red-500 hover:bg-red-600 text-white" : "bg-cyan-600 hover:bg-cyan-700 text-white"}`}>
          {isRunning ? "⏸ বিরাম" : "▶ গণনা শুরু করুন"}
        </button>
        <button onClick={reset} className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold rounded-xl transition-colors">
          🔄 রিসেট
        </button>
      </div>
    </div>
...
function ExampleSection() {
  ...
  const [currentBottle, setCurrentBottle] = useState(99);
  ...
  const startDemo = useCallback(() => {
    ...
    const t = setInterval(() => {
      ...
      setOutputLines(prev => [...prev, `${bottle} bottles`]);
      
      if (bottle <= 97) {
        setOutputLines(prev => [...prev, "..."]);
        clearInterval(t);
        setTimeout(() => {
          setOutputLines(prev => [...prev, "3 bottles", "2 bottles", "1 bottle"]);
          setPhase("done");
        }, 800);
      }
      bottle--;
  ...
  return (
    <div className="space-y-4">
      <button onClick={startDemo} disabled={phase === "looping"}
        ...
        {phase === "idle" || phase === "done" ? "▶ ডেমো চালান" : "⏳ গণনা করছে..."}
      </button>
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-gray-400 text-xs font-mono">bottles.py</span>
          </div>
        </div>
        <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
          <div className="flex">
            ...
            <div><span className="text-purple-400">for</span><span className="text-cyan-400"> bottles</span><span className="text-purple-400"> in</span><span className="text-amber-400"> range</span><span className="text-gray-300">(</span><span className="text-amber-400">99</span><span className="text-gray-300">,</span><span className="text-amber-400"> 0</span><span className="text-gray-300">,</span><span className="text-red-400"> -1</span><span className="text-gray-300">)</span><span className="text-gray-300">:</span></div>
          </div>
          <div className="flex">
            <span className="text-gray-600 text-xs w-6 text-right mr-3 select-none">2</span>
            <div className="pl-4"><span className="text-purple-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">bottles</span><span className="text-gray-300">,</span><span className="text-green-400"> "bottles"</span><span className="text-gray-300">)</span></div>
          </div>
        </div>
        ...
        <div className="px-4 pb-3 bg-gray-900">
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-gray-500 font-bold">Current:</span>
            <motion.div key={currentBottle} initial={{ scale: 1.3 }} animate={{ scale: 1 }}
              className="flex items-center gap-1 px-2 py-1 bg-cyan-900/30 border border-cyan-500/30 rounded-lg">
              <span className="text-sm">🧃</span>
              <span className="text-xs font-bold text-cyan-400">{currentBottle}</span>
            </motion.div>
          </div>
        </div>
        ...
        <div className="border-t border-gray-700 px-4 py-3 bg-gray-950">
          ...
          <div className="font-mono text-xs space-y-0.5 max-h-[120px] overflow-y-auto">
            {outputLines.length === 0 ? (
              <p className="text-gray-600 italic">Output এখানে দেখা যাবে...</p>
            ) : (
              outputLines.map((line, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                  className={line === "..." ? "text-gray-500" : "text-green-400"}>
                  {line}
                </motion.div>
              ))
            )}
        ...
        {phase === "done" && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-bold text-green-700">The Loop starts at 99 and decreases by 1 each time!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export function Ch4_99Bottles({
  ...
  const handleRun = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      for (const line of lines) {
        ...
        const forMatch = t.match(/^for\s+(\w+)\s+in\s+range\((\d+),\s*(\d+),\s*(-?\d+)\)\s*:/);
        if (forMatch) {
          ...
          if (step === 0) {
            out.push("❌ ValueError: range() arg 3 must not be zero");
          } else if (step > 0) {
            ...
            for (let i = start; i > end; i += step) {
              if (i === 1) out.push(`${i} bottle`);
              else out.push(`${i} bottles`);
            }
          }
        } else {
          out.push(`>>> ${t}`);
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      let a = "";
      if (q.includes("Countdown") || q.includes("countdown")) { a = "Countdown Loop হলো এমন loop যেটা বড় সংখ্যা থেকে ছোট সংখ্যায় গণনা করে। 99 Bottles-এ ৯৯ থেকে ১ পর্যন্ত উল্টোভাবে গণনা হয়!"; }
      else if (q.includes("range(99")) { a = "range(99, 0, -1) মানে: ৯৯ থেকে শুরু করো, ০-এ আসো (কিন্তু ০ অন্তর্ভুক্ত নয়), এবং -1 step দিয়ে উল্টোদিকে যাও!"; }
      else if (q.includes("-1")) { a = "-1 step মানে loop উল্টোদিকে চলবে। প্রতিবার সংখ্যা ১ করে কমবে: 99 → 98 → 97 → ... → 1!"; }
      else if (q.includes("বন্ধ") || q.includes("stop")) { a = "Loop বন্ধ হয় যখন সংখ্যা ০ হয়। range(99, 0, -1) ০ অন্তর্ভুক্ত করে না, তাই loop ১ পর্যন্ত চলে!"; }
      else if (q.includes("উল্টো") || q.includes("backward")) { a = "for Loop দিয়ে উল্টোভাবে গণনা করতে range()-এ negative step (-1) ব্যবহার করো। এটি বড় সংখ্যা থেকে ছোট সংখ্যায় গণনা করে!"; }
      else { a = "ভালো প্রশ্ন! Countdown Loop হলো উল্টোদিকে গণনা করার উপায়। নিচের suggested questions দেখো!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">99 Bottles Countdown কী?</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              <span className="font-bold text-cyan-600">Loop</span> দিয়ে <span className="font-bold text-purple-600">উল্টোভাবে</span> গণনা করা যায়!
            </p>
            <p className="text-gray-700 text-[15px] leading-relaxed mt-2">
              ৯৯টি বোতল থেকে শুরু করে <span className="font-bold text-red-600">একটি বোতল</span> করে কমিয়ে নামানো যায় —
              Python নিজেই গণনা করে!
            </p>
          </motion.div>

          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">⬇️ Countdown Flow</p>
              <CountdownFlowAnimation />
            </div>
          </motion.div>

          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🧃 Interactive Bottle Shelf</p>
              <BottleShelfAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> 99 Bottles
                Python-এর countdown loop-এর চমৎকার উদাহরণ —
                <span className="font-bold text-red-600"> -1</span> step দিয়ে উল্টোদিকে গণনা হয়!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">Countdown Loop আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "🧃", label: "Juice Bottles", desc: "শেলফ থেকে বোতল নামছে", code: "99 → 0" },
              { icon: "🍎", label: "Apples", desc: "টোকারি থেকে আপেল কমছে", code: "Eating..." },
              { icon: "🎈", label: "Balloons", desc: "বেলুন উড়ে যাচ্ছে", code: "12 → 0" },
              { icon: "🚀", label: "Rocket", desc: "রকেট উৎক্ষেপণ গণনা!", code: "10...1...0" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🚀 Rocket Countdown</p>
              <div className="flex items-center justify-center gap-3">
                {[10, 9, 8, "...", 3, 2, 1].map((n, i) => (
                  <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * i }}
                    className={`text-center ${n === "..." ? "text-gray-500 text-sm" : ""}`}>
                    {n !== "..." && (
                      <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 1, repeat: Infinity, delay: i * 0.2 }}
                        className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${i < 3 ? "bg-red-900/30 border border-red-500/30 text-red-400" : "bg-cyan-900/30 border border-cyan-500/30 text-cyan-400"}`}>
                        {n}
                      </motion.div>
                    )}
                    {n === "..." && <span>...</span>}
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                ...
                ঠিক যেমন রকেট উৎক্ষেপণের সময় ১০ থেকে ১ পর্যন্ত গণনা হয় —
                Loop-ও উল্টোদিকে গণনা করতে পারে!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: 99 Bottles Countdown তৈরি করো!</p>
                  <p className="text-xs text-gray-500 mt-1">bottles.py ফাইলে countdown loop দিয়ে বোতল গণনা করো</p>
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-purple-400">for</span> <span className="text-cyan-400">bottles</span> <span className="text-purple-400">in</span> <span className="text-amber-400">range</span><span className="text-gray-300">(</span><span className="text-amber-400">99</span><span className="text-gray-300">,</span> <span className="text-amber-400">0</span><span className="text-gray-300">,</span> <span className="text-red-400">-1</span><span className="text-gray-300">)</span><span className="text-gray-300">:</span></p>
                <p className="pl-4"><span className="text-purple-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">bottles</span><span className="text-gray-300">,</span> <span className="text-green-400">"bottles"</span><span className="text-gray-300">)</span></p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "for Loop ব্যবহার করো।", code: "for bottles in range(99, 0, -1):" },
              { num: 2, text: "range(99, 0, -1) ব্যবহার করো।", code: "# ৯৯ থেকে ১ পর্যন্ত" },
              { num: 3, text: "প্রতিবার bottles সংখ্যা প্রিন্ট করো।", code: '    print(bottles, "bottles")' },
              { num: 4, text: "কোড Run করো এবং countdown দেখো।", code: "# 99 → 98 → ... → 1" },
            ].map((step, i) => (
              <motion.div key={i} variants={FADE_UP} onClick={() => toggleStep(i)}
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 bg-gray-900 rounded-lg p-2.5 font-mono text-xs">
                    <span className={step.code.includes("#") ? "text-gray-400" : step.code.includes("print") ? "text-green-400" : step.code.includes("for") ? "text-purple-400" : "text-cyan-400"}>{step.code}</span>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={FADE_UP} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-rose-800 via-pink-800 to-purple-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 4: 99 Bottles</p>
                  <p className="text-white/50 text-xs mt-1">Countdown loops with range()</p>
                </div>
              </div>
            </div>
            <div className="absolute top-3 right-3 px-2.5 py-1 bg-black/70 backdrop-blur-sm rounded-lg text-white text-xs font-mono">5:50</div>
            <AnimatePresence>
              {videoComplete && (
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-gray-500 w-10 text-right">5:50</span>
          </div>
        </div>
      </motion.section>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🏆</span>
              <p className="text-sm font-bold text-purple-800">Goal: 99 Bottles countdown তৈরি করো!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-purple-400">for bottles in range(99, 0, -1):</p>
              <p className="text-green-400 pl-4">print(bottles, "bottles")</p>
            </div>
          </motion.div>
            ...
            <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
              ...
              <div className="flex bg-gray-900">
                ...
                <textarea value={editorCode} onChange={(e) => { setEditorCode(e.target.value); setShowEncourage(false); }}
                  placeholder={'for bottles in range(99, 0, -1):\n    print(bottles, "bottles")'}
            ...
            {solutionOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="overflow-hidden">
                <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-md">
                  ...
                  <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
                    {[
                      { parts: [{ c: "text-purple-400", t: "for" }, { c: "text-cyan-400", t: " bottles" }, { c: "text-purple-400", t: " in" }, { c: "text-amber-400", t: " range" }, { c: "text-gray-300", t: "(99, 0, -1):" }] },
                      { parts: [{ c: "text-purple-400", t: "    print" }, { c: "text-gray-300", t: "(" }, { c: "text-cyan-400", t: "bottles" }, { c: "text-gray-300", t: "," }, { c: "text-green-400", t: ' "bottles"' }, { c: "text-gray-300", t: ")" }] },
                    ].map((item, i) => (
                  ...
                  <div className="px-4 pb-4 bg-gray-900">
                    <div className="p-3 bg-gray-800 rounded-lg font-mono text-xs">
                      ...
                      <span className="text-green-400 font-bold">99 bottles → 1 bottle (99 lines)</span>
                    </div>
                  </div>
                </div>
              </motion.div>
            ...
            <div className="h-72 overflow-y-auto p-4 space-y-3">
              {aiMessages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-xs text-gray-500 max-w-[240px] mb-4 leading-relaxed">99 Bottles সম্পর্কে যেকোনো প্রশ্ন করো! নিচের যেকোনো প্রশ্নে ক্লিক করো।</p>
