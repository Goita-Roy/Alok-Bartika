...
const LESSON_ID = "ch3-type-error";
const SOLUTION_CODE = `age = 15\n\nprint("Age: " + str(age))`;

const SUGGESTED_QUESTIONS = [
  "TypeError কী?",
  "Type mismatch কেন হয়?",
  "str() function কী করে?",
  "int() আর float() কখন লাগে?",
  "TypeError কীভাবে ঠিক করবো?",
];
...
const HINTS = [
  "Python-এ ভিন্ন ধরনের data সরাসরি যোগ করা যায় না — str আর int আলাদা!",
  "str() function দিয়ে number কে string-এ রূপান্তর করো।",
  "int() function দিয়ে string কে number-এ রূপান্তর করো।",
  "Error message পড়ো — এটি বলে কোন দুটি type compatible না!",
];
...
/* ── Type Error Learning Flow ── */
function TypeFlowAnimation() {
  const [step, setStep] = useState(-1);

  const steps = [
    { icon: "🔤", label: "স্ট্রিং", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "🔢", label: "সংখ্যা", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "➕", label: "যোগ?", color: "text-red-400", bg: "bg-red-900/30 border-red-500/50" },
    { icon: "❌", label: "টাইপ ত্রুটি", color: "text-red-400", bg: "bg-red-900/30 border-red-500/50" },
    { icon: "🔄", label: "রূপান্তর", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
        ...
        {step >= 5 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              ...
              <p className="text-sm font-bold text-green-400">Types converted — Code runs!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        ...
        <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
          <div className="flex">
            ...
            <div><span className="text-cyan-400">age</span><span className="text-yellow-400"> = </span><span className="text-amber-400">15</span></div>
          </div>
          <div className="flex">
            ...
            <div>
              ...
              {!showFix ? (
                <>
                  ...
                  <motion.span animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 0.8, repeat: Infinity }} className="text-red-400 font-bold"> + </motion.span>
          ...
          {showFix && (
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-2 pt-1">
              ...
              <span className="text-green-400 text-xs">+ str(age) converts number to string!</span>
            </motion.div>
          )}
        </div>
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="px-4 pb-4 bg-gray-900">
              <div className="p-3 bg-red-900/30 border border-red-500/50 rounded-lg font-mono text-xs space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-red-300">can only concatenate str (not "int") to str</span>
                </div>
                <p className="text-red-300/70 ml-7">Use str(age) to convert!</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="px-4 pb-4 bg-gray-900">
              <div className="p-3 bg-green-900/30 border border-green-500/50 rounded-lg font-mono text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-green-300">Age: 15</span>
                </div>
              </div>
            </motion.div>
          )}
...
export function Ch3TypeError({
    ...
    setTimeout(() => {
      ...
      for (const line of editorCode.split("\n")) {
        ...
        if (eqIdx > 0 && !t.startsWith("print")) {
          ...
          // Detect string + int concatenation (TypeError)
          const concatMatch = inner.match(/^"([^"]*)"\s*\+\s*(.+)$/);
          if (concatMatch) {
            const rightSide = concatMatch[2].trim();
            if (vars[rightSide] !== undefined && !isNaN(Number(vars[rightSide]))) {
              // Check if wrapped in str()
              if (t.includes("str(")) {
                out.push(concatMatch[1] + vars[rightSide]);
              } else {
                out.push(`❌ TypeError: can only concatenate str (not "int") to str`);
                out.push(`   Use str(${rightSide}) to convert!`);
                break;
              }
            } else if (vars[rightSide] !== undefined) {
              out.push(concatMatch[1] + vars[rightSide]);
            } else if (rightSide.includes("str(") && rightSide.includes(")")) {
              const varName = rightSide.replace("str(", "").replace(")", "").trim();
              if (vars[varName] !== undefined) {
                out.push(concatMatch[1] + vars[varName]);
              } else {
                out.push(`❌ NameError: name '${varName}' is not defined`);
                break;
              }
            } else {
              out.push(concatMatch[1] + rightSide.replace(/"/g, ""));
            }
            ...
            // Check for str() wrapping
            if (inner.startsWith("str(") && inner.endsWith(")")) {
              const vn = inner.slice(4, -1).trim();
              if (vars[vn] !== undefined) {
                out.push(vars[vn]);
              } else {
                out.push(vn);
              }
            } else if (inner.startsWith("int(") && inner.endsWith(")")) {
              const vn = inner.slice(4, -1).trim();
              if (vars[vn] !== undefined && !isNaN(Number(vars[vn]))) {
                out.push(String(Number(vars[vn])));
              } else {
                out.push(`❌ ValueError: invalid literal for int()`);
                break;
              }
            } else {
              out.push(`>>> ${t}`);
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      let a = "";
      if (q.includes("TypeError") || q.includes("type error")) { a = "TypeError হলো Python-এ এমন ত্রুটি যখন তুমি ভিন্ন ধরনের data-এর সাথে operation করো। যেমন: string আর number যোগ করলে TypeError হয়!"; }
      else if (q.includes("mismatch") || q.includes("কেন হয়")) { a = "Type mismatch হয় কারণ Python জানে না 'Hello' আর 5 কীভাবে যোগ করতে হয়। তাই Python বলে: এই দুটো type compatible না!"; }
      else if (q.includes("str()") || q.includes("string")) { a = "str() function number কে string-এ রূপান্তর করে। যেমন: str(15) দিলে '15' হয়। তারপর string আর string যোগ করা যায়!"; }
      else if (q.includes("int()") || q.includes("float()")) { a = "int() string কে integer-এ, float() string কে decimal number-এ রূপান্তর করে। যেমন: int('15') দিলে 15 হয়।"; }
      else if (q.includes("ঠিক করবো") || q.includes("fix")) { a = "TypeError ঠিক করতে: ১) Error message পড়ো, ২) কোন দুটি type incompatible তা দেখো, ৩) str(), int(), বা float() দিয়ে রূপান্তর করো, ৪) আবার Run করো!"; }
      else { a = "ভালো প্রশ্ন! TypeError হলো data type mismatch সংক্রান্ত ত্রুটি। নিচের suggested questions দেখো!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">TypeError কী?</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              Python-এ প্রতিটি data-এর <span className="font-bold text-indigo-600">Type</span> আছে — যেমন <span className="font-bold text-blue-600">str</span> (string), <span className="font-bold text-amber-600">int</span> (number), <span className="font-bold text-purple-600">float</span> (decimal)।
            </p>
            <p className="text-gray-700 text-[15px] leading-relaxed mt-2">
              যদি ভিন্ন ধরনের data নিয়ে <span className="font-bold text-red-600">operation</span> করো, Python <span className="font-bold text-red-600">TypeError</span> দেখায়!
            </p>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                { icon: "🔤", title: "স্ট্রিং", desc: '"Hello" — টেক্সট ডেটা', color: "from-blue-50 to-cyan-50 border-blue-200" },
                { icon: "🔢", title: "পূর্ণসংখ্যা", desc: "15 — পূর্ণ সংখ্যা", color: "from-amber-50 to-orange-50 border-amber-200" },
                { icon: "🔣", title: "দশমিক", desc: "3.14 — দশমিক সংখ্যা", color: "from-purple-50 to-pink-50 border-purple-200" },
              ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🎬 TypeError Flow</p>
              <TypeFlowAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> TypeError
                Python-কে বলছে "তুমি string আর number একসাথে যোগ করতে পারো না!" — তাই
                <span className="font-bold text-blue-600"> str()</span> দিয়ে রূপান্তর করো!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">TypeError আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "🔌", label: "ভুল Plug", desc: "ভুল সাইজের plug", code: "Wrong fit" },
              { icon: "🔧", label: "ভুল Wrench", desc: "ভুল সাইজের স্ক্রু", code: "Size mismatch" },
              { icon: "🧩", label: "Puzzle Piece", desc: "ভুল জায়গায় পিস", code: "Doesn't fit" },
              { icon: "⛽", label: "ভুল Fuel", desc: "ডিজেল গাড়িতে পেট্রোল", code: "Wrong type" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🔌 ভুল plug-এর উদাহরণ</p>
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="px-4 py-3 bg-red-900/30 border border-red-500/30 rounded-xl">
                    <p className="text-red-400 font-mono text-sm">"Hello" + 15</p>
                  </motion.div>
                  <p className="text-xs text-gray-500 mt-2">❌ Mismatch!</p>
                </div>
                ...
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className="px-4 py-3 bg-green-900/30 border border-green-500/30 rounded-xl">
                    <p className="text-green-400 font-mono text-sm">"Hello" + str(15)</p>
                  </motion.div>
                  <p className="text-xs text-gray-500 mt-2">✅ Fits!</p>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                ...
                ঠিক যেমন ভুল সাইজের plug কাজ করে না —
                ভিন্ন type-এর data একসাথে কাজ করে না!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: TypeError ঠিক করো!</p>
                  <p className="text-xs text-gray-500 mt-1">type_error.py ফাইলে type conversion করো</p>
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-cyan-400">age</span> <span className="text-yellow-400">=</span> <span className="text-amber-400">15</span></p>
                <p></p>
                <p><span className="text-purple-400">print</span><span className="text-gray-300">(</span><span className="text-green-400">"Age: "</span><span className="text-red-400"> + </span><span className="text-cyan-400">age</span><span className="text-gray-300">)</span></p>
                <p className="text-red-400 text-xs mt-2">❌ TypeError: can only concatenate str (not "int") to str</p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "প্রদত্ত কোডটি Run করো।", code: 'print("Age: " + age)' },
              { num: 2, text: "TypeError message মনোযোগ দিয়ে পড়ো।", code: "# TypeError" },
              { num: 3, text: "কোন type mismatch হচ্ছে তা দেখো।", code: "# str + int = ?" },
              { num: 4, text: "str() দিয়ে convert করো এবং আবার Run করো।", code: 'print("Age: " + str(age))' },
            ].map((step, i) => (
              <motion.div key={i} variants={FADE_UP} onClick={() => toggleStep(i)}
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 bg-gray-900 rounded-lg p-2.5 font-mono text-xs">
                    <span className={step.code.includes("TypeError") || step.code.includes("#") || step.code.includes("+ age") ? "text-red-400" : "text-green-400"}>{step.code}</span>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={FADE_UP} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-rose-800 via-pink-800 to-purple-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 3: TypeError</p>
                  <p className="text-white/50 text-xs mt-1">Type conversion and compatibility</p>
                </div>
              </div>
            </div>
            <div className="absolute top-3 right-3 px-2.5 py-1 bg-black/70 backdrop-blur-sm rounded-lg text-white text-xs font-mono">5:20</div>
            <AnimatePresence>
              {videoComplete && (
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-gray-500 w-10 text-right">5:20</span>
          </div>
        </div>
      </motion.section>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-sm font-bold text-purple-800">Goal: TypeError ঠিক করো!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-red-400">print("Age: " + age) <span className="text-gray-500"># TypeError!</span></p>
              <p className="text-green-400 mt-2">print("Age: " + str(age)) <span className="text-gray-500"># Fixed!</span></p>
            </div>
          </motion.div>
            ...
            <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
              <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-2 text-gray-400 text-xs font-mono">type_error.py</span>
                </div>
            ...
            {solutionOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="overflow-hidden">
                <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-md">
                  ...
                  <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
                    {[
                      { parts: [{ c: "text-cyan-400", t: "age" }, { c: "text-yellow-400", t: " = " }, { c: "text-amber-400", t: "15" }] },
                      { parts: [] },
                      { parts: [{ c: "text-purple-400", t: "print" }, { c: "text-gray-300", t: "(" }, { c: "text-green-400", t: '"Age: "' }, { c: "text-gray-300", t: " + " }, { c: "text-purple-400", t: "str" }, { c: "text-gray-300", t: "(" }, { c: "text-cyan-400", t: "age" }, { c: "text-gray-300", t: "))" }] },
                    ].map((item, i) => (
                  ...
                  <div className="px-4 pb-4 bg-gray-900">
                    <div className="p-3 bg-gray-800 rounded-lg font-mono text-xs"><span className="text-gray-500">Output → </span><span className="text-green-400 font-bold">Age: 15</span></div>
                  </div>
                </div>
              </motion.div>
            )}
            ...
            <div className="h-72 overflow-y-auto p-4 space-y-3">
              {aiMessages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-xs text-gray-500 max-w-[240px] mb-4 leading-relaxed">TypeError সম্পর্কে যেকোনো প্রশ্ন করো! নিচের যেকোনো প্রশ্নে ক্লিক করো।</p>
