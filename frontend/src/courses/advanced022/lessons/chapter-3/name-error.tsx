...
const LESSON_ID = "ch3-name-error";
const SOLUTION_CODE = `name = "Rahim"\n\nprint(name)`;

const SUGGESTED_QUESTIONS = [
  "NameError কী?",
  "Variable Name কেন গুরুত্বপূর্ণ?",
  "Python case-sensitive কেন?",
  "Variable আগে তৈরি করতে হয় কেন?",
  "NameError কীভাবে ঠিক করবো?",
];
...
const MOTIVATIONAL = [
  ...
  "অসাধারণ চেষ্টা!",
  "তুমি সঠিক পথে আছো!",
  "বাহ! NameError master!",
];

const HINTS = [
  "Variable-এর বানান যাচাই করো — সঠিক কি লেখা আছে?",
  "Python case-sensitive — Name আর name আলাদা!",
  "Variable আগে তৈরি করো, তারপর ব্যবহার করো।",
  "সব জায়গায় একই Variable Name ব্যবহার করো!",
];
...
/* ── NameError Learning Flow ── */
function NameFlowAnimation() {
  const [step, setStep] = useState(-1);

  const steps = [
    { icon: "📦", label: "Create Var", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "🔤", label: "Wrong Name", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "❌", label: "NameError", color: "text-red-400", bg: "bg-red-900/30 border-red-500/50" },
    { icon: "🛠️", label: "Fix Name", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
      ...
      <div className="flex items-center justify-center gap-1.5 sm:gap-3 flex-wrap">
        {steps.map((s, i) => (
          ...
          >
            ...
            <p className={`text-[9px] sm:text-[10px] font-bold mt-2 ${i <= step ? s.color : "text-gray-600"}`}>{s.label}</p>
          </motion.div>
        ))}
      </div>
        ...
        {step >= 4 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              ...
              <p className="text-sm font-bold text-green-400">Variable name corrected!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function ExampleSection() {
  ...
  const startDemo = useCallback(() => {
    ...
    setTimeout(() => { setShowError(true); setPhase("error"); }, 1200);
    setTimeout(() => { setShowFix(true); setPhase("fixing"); }, 3000);
    setTimeout(() => { setShowError(false); setPhase("done"); }, 4500);
  }, []);

  return (
    <div className="space-y-4">
      <button onClick={startDemo} disabled={phase !== "idle" && phase !== "done"}
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        ...
        <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
          <div className="flex">
            ...
            <div><span className="text-cyan-400">name</span><span className="text-yellow-400"> = </span><span className="text-green-400">"Rahim"</span></div>
          </div>
          <div className="flex">
          ...
          <div className="flex">
            ...
            <div>
              ...
              {!showFix ? (
                <>
                  <motion.span animate={{ opacity: [1, 0.4, 1] }} transition={{ duration: 0.8, repeat: Infinity }} className="text-red-400 font-bold">nmae</motion.span>
                </>
              ) : (
          ...
          {showFix && (
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-2 pt-1">
              ...
              <span className="text-green-400 text-xs">nmae → name (Fixed!)</span>
            </motion.div>
          )}
        </div>
        ...
        <AnimatePresence>
          {showError && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="px-4 pb-4 bg-gray-900">
              <div className="p-3 bg-red-900/30 border border-red-500/50 rounded-lg font-mono text-xs space-y-1">
                <div className="flex items-center gap-2">
                  <span className="text-red-400 text-lg">❌</span>
                  <span className="text-red-400 font-bold">NameError:</span>
                  <span className="text-red-300">name 'nmae' is not defined</span>
                </div>
                <p className="text-red-300/70 ml-7">Did you mean: 'name'?</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {phase === "done" && !showError && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="px-4 pb-4 bg-gray-900">
              <div className="p-3 bg-green-900/30 border border-green-500/50 rounded-lg font-mono text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-green-300">Rahim</span>
                </div>
              </div>
            </motion.div>
          )}
...
export function Ch3NameError({
    ...
    setTimeout(() => {
      ...
      for (const line of editorCode.split("\n")) {
        ...
        if (eqIdx > 0 && !t.startsWith("print")) {
          const vn = t.slice(0, eqIdx).trim();
          let val = t.slice(eqIdx + 1).trim();
          if ((val.startsWith('"') && val.endsWith('"')) || (val.startsWith("'") && val.endsWith("'"))) val = val.slice(1, -1);
          vars[vn] = val;
        } else if (t.startsWith("print(") && t.endsWith(")")) {
          const inner = t.slice(6, -1).trim();
          if (vars[inner] !== undefined) { out.push(vars[inner]); }
          else if ((inner.startsWith('"') && inner.endsWith('"')) || (inner.startsWith("'") && inner.endsWith("'"))) { out.push(inner.slice(1, -1)); }
          else {
            const found = Object.keys(vars).find(v => v.toLowerCase() === inner.toLowerCase() && v !== inner);
            if (found) { out.push(`❌ NameError: name '${inner}' is not defined`); out.push(`   Did you mean: '${found}'?`); break; }
            else { out.push(`❌ NameError: name '${inner}' is not defined`); break; }
          }
      ...
      if (hasSuccess && !hasError) { setEncourage(MOTIVATIONAL[Math.floor(Math.random() * MOTIVATIONAL.length)]); setShowEncourage(true); setTimeout(() => setShowEncourage(false), 3500); setStepsDone([true, true, true, true]); }
    }, 650);
  }, [editorCode]);
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      let a = "";
      if (q.includes("NameError") || q.includes("name error")) { a = "NameError হলো Python-এ এমন ত্রুটি যখন Python কোনো Variable বা function name খুঁজে পায় না। সাধারণত Variable ভুল বানান লেখা বা আগে তৈরি না করা থাকলে এটি হয়!"; }
      else if (q.includes("Variable Name") || q.includes("variable name") || q.includes("গুরুত্বপূর্ণ")) { a = "Variable Name গুরুত্বপূর্ণ কারণ Python সরাসরি name দিয়েই খুঁজে বের করে। 'name' আর 'nmae' আলাদা জিনিস Python-এর কাছে!"; }
      else if (q.includes("case-sensitive") || q.includes("Case")) { a = "Python case-sensitive — অর্থাৎ Name আর name আলাদা Variable! Name = 'Rahim' এবং name = 'Rahim' দুটো আলাদা Variable। সবসময় একই বানান ব্যবহার করো!"; }
      else if (q.includes("আগে তৈরি") || q.includes("কেন") || q.includes("create")) { a = "Variable আগে তৈরি করতে হয় কারণ Python-কে জানতে হয় Variable আছে কিনা। যদি নাম না জানে, কীভাবে মান বের করবে? তাই আগে name = 'Rahim' লিখো, তারপর print(name) লিখো!"; }
      else if (q.includes("ঠিক করবো") || q.includes("fix")) { a = "NameError ঠিক করতে: ১) Error message পড়ো, ২) ভুল Variable Name খুঁজো, ৩) সঠিক বানান দিয়ে প্রতিস্থাপন করো, ৪) আবার Run করো!"; }
      else { a = "ভালো প্রশ্ন! NameError হলো Variable Name সংক্রান্ত ত্রুটি। নিচের suggested questions দেখো!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">NameError কী?</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              প্রতিটি <span className="font-bold text-cyan-600">Variable</span> ব্যবহারের আগে তৈরি করতে হয়।
              Python যদি Variable বা function name <span className="font-bold text-red-600">খুঁজে না পায়</span>, তাহলে <span className="font-bold text-red-600">NameError</span> দেখায়!
            </p>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                { icon: "❌", title: "Not Created", desc: "Variable তৈরি হয়নি", color: "from-red-50 to-pink-50 border-red-200" },
                { icon: "❌", title: "Misspelled", desc: "ভুল বানান", color: "from-amber-50 to-orange-50 border-amber-200" },
                { icon: "❌", title: "Wrong Case", desc: "Name ≠ name", color: "from-purple-50 to-pink-50 border-purple-200" },
              ].map((item, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 * i, duration: 0.5 }} whileHover={{ scale: 1.04, y: -4 }}
                  className={`bg-gradient-to-br ${item.color} border-2 rounded-2xl p-4 cursor-default`}>
                  <motion.span animate={{ y: [0, -4, 0] }} transition={{ duration: 2, repeat: Infinity, delay: i * 0.3 }} className="text-3xl block mb-2">{item.icon}</motion.span>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🎬 NameError Flow</p>
              <NameFlowAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> NameError
                Python-কে বলছে "আমি {`'nmae'`} খুঁজে পাইনি!" — তাই সঠিক বানান লিখো!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">NameError আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "📞", label: "ভুল নম্বর", desc: "ভুল মানুষকে ফোন করা", code: "Dial wrong #?" },
              { icon: "🏷️", label: "ভুল নাম", desc: "নামের ট্যাগ ভুল", code: "Tag: Rhaim" },
              { icon: "📦", label: "ভুল বাক্স", desc: "ভুল বাক্স খুঁজছো", code: "Find box?" },
              { icon: "📖", label: "ভুল বানান", desc: "বন্ধুর নাম ভুল", code: "Rahim → Rhaim" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">📞 ভুল নাম ডায়ালের উদাহরণ</p>
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="px-4 py-3 bg-red-900/30 border border-red-500/30 rounded-xl">
                    <p className="text-red-400 font-mono text-sm">"Rhaim"</p>
                  </motion.div>
                  <p className="text-xs text-gray-500 mt-2">❌ Confused!</p>
                </div>
                ...
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} className="px-4 py-3 bg-green-900/30 border border-green-500/30 rounded-xl">
                    <p className="text-green-400 font-mono text-sm">"Rahim"</p>
                  </motion.div>
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                ...
                ঠিক যেমন ভুল নামে ফোন করলে মানুষ বোঝে না —
                Python-ও Variable খুঁজে পায় না!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: NameError ঠিক করো!</p>
                  <p className="text-xs text-gray-500 mt-1">name_error.py ফাইলে Variable Name ঠিক করো</p>
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                ...
                <p></p>
                <p><span className="text-purple-400">print</span><span className="text-gray-300">(</span><span className="text-red-400">nmae</span><span className="text-gray-300">)</span></p>
                <p className="text-red-400 text-xs mt-2">❌ NameError: name 'nmae' is not defined</p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "প্রদত্ত কোডটি Run করো।", code: 'print(nmae)' },
              { num: 2, text: "NameError message মনোযোগ দিয়ে পড়ো।", code: "# NameError" },
              { num: 3, text: "ভুল Variable Name খুঁজে বের করো।", code: "# nmae = ?" },
              { num: 4, text: "বানান ঠিক করো এবং আবার Run করো।", code: 'print(name)' },
            ].map((step, i) => (
              <motion.div key={i} variants={FADE_UP} onClick={() => toggleStep(i)}
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 bg-gray-900 rounded-lg p-2.5 font-mono text-xs">
                    <span className={step.code.includes("#") || step.code.includes("nmae") ? "text-red-400" : "text-green-400"}>{step.code}</span>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={FADE_UP} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-red-800 via-pink-800 to-rose-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 3: NameError</p>
                  <p className="text-white/50 text-xs mt-1">Find and fix variable name errors</p>
                </div>
              </div>
            </div>
            <div className="absolute top-3 right-3 px-2.5 py-1 bg-black/70 backdrop-blur-sm rounded-lg text-white text-xs font-mono">5:40</div>
            <AnimatePresence>
              {videoComplete && (
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-gray-500 w-10 text-right">5:40</span>
          </div>
        </div>
      </motion.section>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-sm font-bold text-purple-800">Goal: Variable Name ঠিক করো!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-red-400">print(nmae) <span className="text-gray-500"># NameError!</span></p>
              <p className="text-green-400 mt-2">print(name) <span className="text-gray-500"># Fixed!</span></p>
            </div>
          </motion.div>
            ...
            <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
              <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-2 text-gray-400 text-xs font-mono">name_error.py</span>
                </div>
            ...
            {solutionOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="overflow-hidden">
                <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-md">
                  ...
                  <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
                    {[
                      { parts: [{ c: "text-cyan-400", t: "name" }, { c: "text-yellow-400", t: " = " }, { c: "text-green-400", t: '"Rahim"' }] },
                      { parts: [] },
                      { parts: [{ c: "text-purple-400", t: "print" }, { c: "text-gray-300", t: "(" }, { c: "text-cyan-400", t: "name" }, { c: "text-gray-300", t: ")" }] },
                    ].map((item, i) => (
                  ...
                  <div className="px-4 pb-4 bg-gray-900">
                    <div className="p-3 bg-gray-800 rounded-lg font-mono text-xs"><span className="text-gray-500">Output → </span><span className="text-green-400 font-bold">Rahim</span></div>
                  </div>
                </div>
              </motion.div>
            )}
            ...
            <div className="h-72 overflow-y-auto p-4 space-y-3">
              {aiMessages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-xs text-gray-500 max-w-[240px] mb-4 leading-relaxed">NameError সম্পর্কে যেকোনো প্রশ্ন করো! নিচের যেকোনো প্রশ্নে ক্লিক করো।</p>
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={FADE_UP} className="flex items-center justify-between pt-6 border-t border-gray-200">
        {!isCompleted ? (
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => onComplete(LESSON_ID)}
            ...
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>সম্পন্ন হিসেবে চিহ্নিত করুন
          </motion.button>
        ) : (
          <span className="flex items-center gap-2.5 px-7 py-3.5 bg-green-50 text-green-700 text-sm font-extrabold rounded-2xl border-2 border-green-200">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>সম্পন্ন!
          </span>
        )}
        {hasNext && (
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={onNext}
            ...
            পরবর্তী পাঠ<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
          </motion.button>
        )}
      </motion.div>
    </div>
