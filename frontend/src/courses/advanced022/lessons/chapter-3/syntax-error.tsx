...
const LESSON_ID = "ch3-syntax-error";
const SOLUTION_CODE = `print("Welcome to Python!")`;

const SUGGESTED_QUESTIONS = [
  "Syntax Error কী?",
  "Python Syntax কী?",
  "Quotation Mark কেন দরকার?",
  "Parenthesis ভুল হলে কী হয়?",
  "Syntax Error কীভাবে ঠিক করবো?",
];
...
const HINTS = [
  ...
  "Quotation marks খুঁজে বের করো — একটি কি কমে আছে?",
  "Parentheses এবং colons যাচাই করো।",
  "Python খুব কঠোর Syntax Rule মানে — প্রতিটি symbol সঠিক জায়গায় থাকতে হবে!",
];
...
/* ── Syntax Learning Flow ── */
function SyntaxFlowAnimation() {
  const [step, setStep] = useState(-1);

  const steps = [
    { icon: "✍️", label: "কোড লেখো", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "❌", label: "সিনট্যাক্স ত্রুটি", color: "text-red-400", bg: "bg-red-900/30 border-red-500/50" },
    { icon: "🔍", label: "বার্তা পড়ো", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "🛠️", label: "ভুল সংশোধন করো", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
    { icon: "✅", label: "পুনরায় চালাও", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
  ];
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-center gap-1.5 sm:gap-3 flex-wrap">
        {steps.map((s, i) => (
          ...
          >
            <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center border-2 ${s.bg}`}>
              <span className="text-xl sm:text-2xl">{s.icon}</span>
            </div>
            <p className={`text-[9px] sm:text-[10px] font-bold mt-2 ${i <= step ? s.color : "text-gray-600"}`}>
              {s.label}
            </p>
          </motion.div>
        ))}
...
/* ── Example Section (animated error + fix) ── */
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-gray-400 text-xs font-mono">example.py</span>
          </div>
        </div>
        <div className="bg-gray-900 p-4 font-mono text-sm">
          ...
          {showFix && (
            ...
            >
              ...
              <span className="text-green-400 text-xs">Missing " added!</span>
            </motion.div>
          )}
        </div>
            ...
            >
              <div className="p-3 bg-red-900/30 border border-red-500/50 rounded-lg font-mono text-xs space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-red-300">EOL while scanning string literal</span>
                </div>
            ...
            >
              <div className="p-3 bg-green-900/30 border border-green-500/50 rounded-lg font-mono text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-green-300">Welcome to Python!</span>
                </div>
              </div>
            </motion.div>
          )}
...
export function Ch3SyntaxError({
  onComplete,
  onNext,
  isCompleted,
  hasNext,
    ...
    setTimeout(() => {
      ...
      for (const line of lines) {
        ...
        if (t.startsWith("print(") && t.endsWith(")")) {
          ...
          if (!isString && inner.length > 0) {
            const hasOpenQuote = inner.includes('"') || inner.includes("'");
            if (hasOpenQuote) {
              out.push(`❌ SyntaxError: EOL while scanning string literal`);
              out.push(`   Missing closing quotation mark`);
              break;
            }
            out.push(inner);
          } else if (isString) {
          ...
          out.push(`   Missing closing parenthesis ')'`);
          break;
        } else if (t.includes('"') && t.split('"').length % 2 === 0) {
          out.push(`❌ SyntaxError: EOL while scanning string literal`);
          out.push(`   Missing closing quotation mark`);
          break;
        } else {
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      let a = "";
      if (q.includes("Syntax Error") || q.includes("syntax error")) {
        a = "Syntax Error হলো Python-এ কোডে ভাঁজা-গুঁজা বা নিয়ম ভাঙার ত্রুটি। যখন কোডে quotation mark, parenthesis বা colon মিসিং থাকে, Python বুঝতে পারে না — তখন Syntax Error দেখায়!";
      } else if (q.includes("Python Syntax") || q.includes("python syntax") || q.includes("Syntax কী")) {
        a = "Python Syntax হলো Python-এর ব্যাকরণ বা কোড লেখার নিয়ম। যেমন: strings-এ quotation mark লাগে, function call-এ parenthesis লাগে, if statement-এ colon লাগে। এগুলো না মানলে Syntax Error হয়!";
      } else if (q.includes("Quotation Mark") || q.includes("quotation mark")) {
        a = "Quotation Mark Python-কে বোঝায় যে এটি একটি String। যেমন: print('Hello') বা print('Hello') — দুটোই চলে! কিন্তু একটি খুলে বন্ধ করতে হবে।";
      } else if (q.includes("Parenthesis") || q.includes("parenthesis")) {
        a = "Parenthesis () লাগে print() function call করতে। যদি একটি parenthesis মিসিং থাকে, Python SyntaxError দেখায়। সবসময় খুলে বন্ধ করো!";
      } else if (q.includes("ঠিক করবো") || q.includes("fix")) {
        a = "Syntax Error ঠিক করতে: ১) Error message পড়ো, ২) Line number দেখো, ৩) কোন symbol মিসিং তা খুঁজো (quotation mark, parenthesis, colon), ৪) Fix করো, ৫) আবার Run করো!";
      } else {
        a = "ভালো প্রশ্ন! Syntax Error হলো Python-এর কোডে নিয়ম ভাঙার ত্রুটি। নিচের suggested questions দেখো!";
      }
    ...
    <div className="space-y-8 pb-12">
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-extrabold text-white tracking-tight">Basic</h2>
          </div>
        </motion.div>

        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">Syntax Error কী?</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              প্রতিটি প্রোগ্রামিং ভাষার নিজস্ব <span className="font-bold text-indigo-600">ব্যাকরণ নিয়ম</span> আছে — একে বলে <span className="font-bold text-indigo-600">Syntax</span>।
            </p>
            <p className="text-gray-700 text-[15px] leading-relaxed mt-2">
              যদি এই নিয়ম ভাঙা হয়, Python কোড <span className="font-bold text-red-600">বুঝতে পারে না</span> — এটিকে বলে <span className="font-bold text-red-600">Syntax Error</span>।
            </p>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                { icon: "❌", title: "Missing Quote", desc: "\"Hello - বন্ধ নেই!", color: "from-red-50 to-pink-50 border-red-200" },
                { icon: "❌", title: "Missing Paren", desc: "print(\"Hi\" - ) নেই!", color: "from-amber-50 to-orange-50 border-amber-200" },
                { icon: "❌", title: "Missing Colon", desc: "if x == 5 - : নেই!", color: "from-purple-50 to-pink-50 border-purple-200" },
              ].map((item, i) => (
                <motion.div
                  key={i}
                  ...
                  className={`bg-gradient-to-br ${item.color} border-2 rounded-2xl p-4 cursor-default`}
                >
                  <motion.span
                    animate={{ y: [0, -4, 0] }}
                  ...
                  <p className="font-mono text-xs text-gray-600 mt-1 bg-white/60 px-2 py-1 rounded-lg">{item.desc}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> Syntax Error
                ভয় পাওয়ার নয় — এটি Python তোমাকে সাহায্য করছে কোড ঠিক করতে!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            <span className="text-3xl">🌍</span>
            <h2 className="text-xl font-extrabold text-white tracking-tight">Real Life Example</h2>
          </div>
        </motion.div>

        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">
            Syntax Error আমাদের দৈনন্দিন জীবনেও ঘটে!
          </motion.p>

          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "📝", label: "ভুল বানান", desc: "বাক্যে ভুল শব্দ", code: "Pythn" },
              { icon: "🚦", label: "ট্রাফিক নিয়ম", desc: "নিয়ম ভাঙলে জরিমানা", code: "Red = Stop" },
              { icon: "🧩", label: "Puzzle", desc: "ভুল জায়গায় পিস", code: "Wrong fit" },
              { icon: "📚", label: "বিরাম চিহ্ন", desc: "ফুলস্টপ নেই", code: "Hello World" },
            ].map((item, i) => (
              <motion.div
                key={i}
                variants={FADE_UP}
            ...
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              ...
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-500 mt-2">❌ Confusing</p>
                </div>
                <motion.div
                  animate={{ x: [0, 8, 0] }}
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-500 mt-2">✅ Clear</p>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                ...
                ঠিক যেমন বাক্যে বিরাম চিহ্ন লাগে — কোডেও
                প্রতিটি symbol সঠিক জায়গায় থাকতে হবে!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      >
        <div className="px-6 py-5 bg-gradient-to-r from-violet-500 to-purple-600">
          <div className="flex items-center gap-3">
            <span className="text-3xl">💻</span>
            <h2 className="text-xl font-extrabold text-white tracking-tight">Example</h2>
          </div>
        </div>
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            <span className="text-3xl">📝</span>
            <h2 className="text-xl font-extrabold text-white tracking-tight">নির্দেশনা</h2>
          </div>
        </motion.div>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: Syntax Error ঠিক করো!</p>
                  <p className="text-xs text-gray-500 mt-1">syntax_error.py ফাইলে Syntax Error খুঁজে বের করো এবং ঠিক করো</p>
                  <div className="flex items-center gap-2 mt-2">
                    {stepsDone.map((d, i) => (
                      <div key={i} className={`h-2 rounded-full transition-all duration-500 ${i === 0 ? "w-16" : i === 1 ? "w-12" : i === 2 ? "w-12" : "w-10"} ${d ? "bg-green-400" : "bg-orange-200"}`} />
                    ))}
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-purple-400">print</span><span className="text-gray-300">(</span><span className="text-green-400">"Welcome to Python!</span></p>
                <p className="text-red-400 text-xs mt-2">❌ SyntaxError: EOL while scanning string literal</p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "প্রদত্ত কোডটি Run করো।", code: 'print("Welcome to Python!' },
              { num: 2, text: "Syntax Error message মনোযোগ দিয়ে পড়ো।", code: "# SyntaxError" },
              { num: 3, text: "মিসিং symbol খুঁজে বের করো।", code: '# Missing " ' },
              { num: 4, text: "কোডটি ঠিক করো এবং আবার Run করো।", code: 'print("Welcome to Python!")' },
            ].map((step, i) => (
              <motion.div
                key={i}
                variants={FADE_UP}
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 bg-gray-900 rounded-lg p-2.5 font-mono text-xs">
                    <span className={step.code.includes("SyntaxError") || step.code.includes("#") ? "text-red-400" : "text-green-400"}>
                      {step.code}
                    </span>
                  </div>
                </div>
      ...
      >
        <div className="px-6 py-5 bg-gradient-to-r from-cyan-500 to-blue-600">
          <div className="flex items-center gap-3">
            <span className="text-3xl">💡</span>
            <h2 className="text-xl font-extrabold text-white tracking-tight">Hint</h2>
          </div>
        </div>
            ...
            {hintOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.4, ease: "easeInOut" }} className="overflow-hidden">
                <div className="mt-4 space-y-3">
                  {HINTS.map((h, i) => (
                    <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={i < hintsRevealed ? { opacity: 1, x: 0 } : { opacity: 0, x: -20 }} transition={{ duration: 0.4 }} className="flex items-start gap-3 p-4 bg-gradient-to-r from-cyan-50 to-blue-50 border border-cyan-100 rounded-xl">
      ...
      >
        <div className="px-6 py-5 bg-gradient-to-r from-rose-500 to-pink-600">
          <div className="flex items-center gap-3">
            <span className="text-3xl">🎥</span>
            <h2 className="text-xl font-extrabold text-white tracking-tight">Walkthrough Video</h2>
          </div>
        </div>
        ...
        <div className="px-6 py-6">
          <div className="relative rounded-2xl overflow-hidden bg-gray-900 aspect-video cursor-pointer group" onClick={handleVideoToggle}>
            <div className="absolute inset-0 bg-gradient-to-br from-red-800 via-pink-800 to-rose-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  <motion.div animate={videoPlaying ? { scale: [1, 1.1, 1] } : {}} transition={{ duration: 1.5, repeat: videoPlaying ? Infinity : 0 }} className="w-20 h-20 mx-auto bg-white/10 backdrop-blur-sm rounded-full flex items-center justify-center border-2 border-white/30 group-hover:bg-white/20 group-hover:scale-110 transition-all duration-300">
                    {videoPlaying ? (
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 3: Syntax Error</p>
                  <p className="text-white/50 text-xs mt-1">Understanding Python syntax rules</p>
                </div>
              </div>
            </div>

            <div className="absolute top-3 right-3 px-2.5 py-1 bg-black/70 backdrop-blur-sm rounded-lg text-white text-xs font-mono">6:00</div>
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-gray-500 w-10 text-right">6:00</span>
          </div>
        </div>
      </motion.section>
      ...
      >
        <div className="px-6 py-5 bg-gradient-to-r from-violet-500 to-purple-700">
          <div className="flex items-center gap-3">
            <span className="text-3xl">🎯</span>
            <h2 className="text-xl font-extrabold text-white tracking-tight">Practice</h2>
          </div>
        </div>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-sm font-bold text-purple-800">Goal: Syntax Error খুঁজে বের করো এবং ঠিক করো!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-red-400">print("Welcome to Python!" <span className="text-gray-500"># missing " and )</span></p>
              <p className="text-green-400 mt-2">print("Welcome to Python!") <span className="text-gray-500"># fixed!</span></p>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
              <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
                <div className="flex items-center gap-2">
                  ...
                  <span className="w-3 h-3 rounded-full bg-yellow-400" />
                  <span className="w-3 h-3 rounded-full bg-green-400" />
                  <span className="ml-2 text-gray-400 text-xs font-mono">syntax_error.py</span>
                </div>
              ...
              <div className="flex bg-gray-900">
                <div className="flex-shrink-0 py-3 pl-3 pr-2 text-right select-none border-r border-gray-700">
                  {Array.from({ length: Math.max(codeLines, 12) }, (_, i) => (
                    <div key={i} className="text-gray-600 text-xs font-mono leading-6 h-6">{i + 1}</div>
                  ))}
                </div>
                <textarea
                  value={editorCode}
                  onChange={(e) => { setEditorCode(e.target.value); setShowEncourage(false); }}
                  placeholder={'print("Welcome to Python!")'}
            ...
            <div className="flex flex-col gap-3">
                ...
                <div className="px-4 py-2 bg-gray-800 border-b border-gray-700 flex items-center gap-2">
                  ...
                  {isRunning && (
                    <span className="ml-auto text-[10px] text-yellow-400 flex items-center gap-1">
                      <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-2.5 h-2.5 border-2 border-yellow-400 border-t-transparent rounded-full" />
                      Running...
                    </span>
                  )}
                </div>
                  ...
                  {consoleLines.length === 0 ? (
                    ...
                    <div className="space-y-0.5">
                      {consoleLines.map((l, i) => (
                        <div key={i} className={l.startsWith("❌") ? "text-red-400" : l.startsWith(">>>") ? "text-gray-400" : l.startsWith("⚠️") ? "text-yellow-400" : "text-green-400"}>{l}</div>
                      ))}
                    </div>
                  )}
                </div>
              ...
              <div className="flex gap-2">
                <motion.button whileTap={{ scale: 0.95 }} onClick={handleRun} disabled={isRunning} className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white text-sm font-bold rounded-xl transition-colors shadow-sm">
                  {isRunning ? (
                    <><motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-4 h-4 border-2 border-white border-t-transparent rounded-full" />চলছে...</>
                  ) : (
              ...
              <AnimatePresence>
                {showEncourage && (
                  <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-xl">
      ...
      >
        <div className="px-6 py-5 bg-gradient-to-r from-amber-500 to-orange-600">
          <div className="flex items-center gap-3">
            <span className="text-3xl">✅</span>
            <h2 className="text-xl font-extrabold text-white tracking-tight">Solution</h2>
          </div>
        </div>
                ...
                <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-md">
                  <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
                    ...
                    <button onClick={copySolution} className="flex items-center gap-1.5 px-2.5 py-1 text-xs text-gray-400 hover:text-white hover:bg-gray-700 rounded-lg transition-colors">
                      {solutionCopied ? (
                        <><svg className="w-3.5 h-3.5 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>Copied!</>
                      ) : (
                  ...
                  <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
                    {[{ parts: [{ c: "text-purple-400", t: "print" }, { c: "text-gray-300", t: "(" }, { c: "text-green-400", t: '"Welcome to Python!"' }, { c: "text-gray-300", t: ")" }] }].map((item, i) => (
                  ...
                  <div className="px-4 pb-4 bg-gray-900">
                    <div className="p-3 bg-gray-800 rounded-lg font-mono text-xs">
                      ...
                      <span className="text-green-400 font-bold">Welcome to Python!</span>
                    </div>
                  </div>
                </div>
              </motion.div>
      ...
      >
        <div className="px-6 py-5 bg-gradient-to-r from-fuchsia-500 to-purple-700">
          <div className="flex items-center gap-3">
            <span className="text-3xl">🤖</span>
            <h2 className="text-xl font-extrabold text-white tracking-tight">AI Coding Companion</h2>
          </div>
        </div>
          ...
          <div className="bg-gradient-to-br from-purple-50 to-fuchsia-50 border border-purple-100 rounded-2xl overflow-hidden shadow-sm">
            ...
            <div className="h-72 overflow-y-auto p-4 space-y-3">
              {aiMessages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  <motion.div animate={{ y: [0, -6, 0] }} transition={{ duration: 2.5, repeat: Infinity }} className="w-16 h-16 bg-purple-100 rounded-full flex items-center justify-center mb-3">
                    <span className="text-3xl">👋</span>
                  </motion.div>
                  <p className="text-sm font-extrabold text-gray-800 mb-1">Hello! I'm PyBot</p>
                  <p className="text-xs text-gray-500 max-w-[240px] mb-4 leading-relaxed">Syntax Error সম্পর্কে যেকোনো প্রশ্ন করো! নিচের যেকোনো প্রশ্নে ক্লিক করো।</p>
                  <div className="space-y-2 w-full max-w-[300px]">
                    {SUGGESTED_QUESTIONS.map((q, i) => (
                      <motion.button key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 * i }} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => { setAiInput(q); setTimeout(() => aiRef.current?.focus(), 50); }} className="w-full text-left px-3.5 py-2.5 bg-white border border-purple-200 rounded-xl text-xs text-purple-700 hover:bg-purple-50 hover:border-purple-300 transition-colors shadow-sm">
                        💡 {q}
                      </motion.button>
                    ))}
                  </div>
                ...
                <>
                  {aiMessages.map((msg, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${msg.role === "user" ? "bg-purple-600 text-white rounded-br-md" : "bg-white border border-gray-200 text-gray-800 rounded-bl-md shadow-sm"}`}>
                        {msg.text}
                      </div>
                    </motion.div>
                  ))}
                  {aiTyping && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start">
                      <div className="bg-white border border-gray-200 rounded-2xl rounded-bl-md px-4 py-3 flex items-center gap-1.5 shadow-sm">
                        {[0, 1, 2].map((i) => (
                          <motion.span key={i} animate={{ y: [0, -4, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay: i * 0.15 }} className="w-2 h-2 bg-purple-400 rounded-full" />
                        ))}
                      </div>
                    </motion.div>
                  )}
            ...
            {aiMessages.length > 0 && aiMessages.length < 6 && (
              <div className="px-4 pb-2 flex gap-1.5 overflow-x-auto">
                {SUGGESTED_QUESTIONS.slice(0, 2).map((q, i) => (
                  <button key={i} onClick={() => { setAiInput(q); setTimeout(() => aiRef.current?.focus(), 50); }} className="flex-shrink-0 px-3 py-1.5 bg-purple-100 text-purple-700 text-[10px] font-semibold rounded-full hover:bg-purple-200 transition-colors">{q}</button>
                ))}
              </div>
            )}
            ...
            <div className="px-4 pb-4">
              <div className="flex items-center gap-2 bg-white border border-purple-200 rounded-xl px-3 py-2.5 focus-within:border-purple-400 focus-within:ring-2 focus-within:ring-purple-100 transition-all shadow-sm">
                ...
                <motion.button whileTap={{ scale: 0.9 }} onClick={aiSend} disabled={!aiInput.trim() || aiTyping} className="flex-shrink-0 w-9 h-9 bg-purple-600 hover:bg-purple-700 disabled:bg-purple-300 text-white rounded-xl flex items-center justify-center transition-colors">
      ...
      {/* ═══════════ Bottom Actions ═══════════ */}
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={FADE_UP} className="flex items-center justify-between pt-6 border-t border-gray-200">
        {!isCompleted ? (
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => onComplete(LESSON_ID)} className="flex items-center gap-2.5 px-7 py-3.5 bg-green-600 text-white text-sm font-extrabold rounded-2xl hover:bg-green-700 transition-all shadow-md hover:shadow-lg">
        ...
        {hasNext && (
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={onNext} className="flex items-center gap-2.5 px-7 py-3.5 bg-indigo-600 text-white text-sm font-extrabold rounded-2xl hover:bg-indigo-700 transition-all shadow-md hover:shadow-lg">
            পরবর্তী পাঠ
