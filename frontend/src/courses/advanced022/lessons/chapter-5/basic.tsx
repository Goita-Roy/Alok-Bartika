...
const LESSON_ID = "ch5-basic";
const SOLUTION_CODE = `fruits = ["Apple", "Banana", "Mango"]\n\nprint(fruits)`;

const SUGGESTED_QUESTIONS = [
  "List কী?",
  "Square Brackets কেন ব্যবহার করি?",
  "List আর Variable-এর পার্থক্য কী?",
  "List-এ একাধিক Item রাখা যায় কীভাবে?",
  "print() দিয়ে List কীভাবে দেখবো?",
];
...
/* ── School Bag Animation ── */
function SchoolBagAnimation() {
  const [step, setStep] = useState(-1);

  const items = [
    { icon: "📚", label: "Book", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "✏️", label: "Pencil", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "📒", label: "Notebook", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
    { icon: "🎨", label: "Color Box", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
    { icon: "📦", label: "School Bag", color: "text-cyan-400", bg: "bg-cyan-900/30 border-cyan-500/50" },
    { icon: "🎒", label: "All Inside!", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
  ];
        ...
        {step >= 5 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              ...
              <p className="text-sm font-bold text-green-400">All items stored in one bag — like a Python List!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function ListFlowAnimation() {
  ...
  const steps = [
    ...
    { icon: "🔍", label: "Easy to Access", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
      ...
      <AnimatePresence>
        {step >= 6 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              <span className="text-lg">🎉</span>
              <p className="text-sm font-bold text-green-400">Lists make programming easy!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function ExampleSection() {
  ...
  const [fruitsInList, setFruitsInList] = useState<string[]>([]);

  const fullCode = 'fruits = ["Apple", "Banana", "Mango"]\n\nprint(fruits)';
    ...
    const typeInterval = setInterval(() => {
      if (idx < fullCode.length) {
        ...
        setTimeout(() => {
          ...
          let fIdx = 0;
          const addFruit = setInterval(() => {
            if (fIdx < fruits.length) {
              setFruitsInList(prev => [...prev, fruits[fIdx]]);
              fIdx++;
            } else {
              clearInterval(addFruit);
              setTimeout(() => {
                setOutputLines(["['Apple', 'Banana', 'Mango']"]);
                setPhase("done");
              }, 500);
            }
          }, 600);
  ...
  return (
    <div className="space-y-4">
      <button onClick={startDemo} disabled={phase === "typing" || phase === "running"}
        ...
        {phase === "idle" || phase === "done" ? "▶ ডেমো চালান" : phase === "typing" ? "⌨️ টাইপ করা হচ্ছে..." : "⏳ লিস্ট তৈরি হচ্ছে..."}
      </button>
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-gray-400 text-xs font-mono">lists.py</span>
          </div>
        </div>
            ...
            <div key={i} className="flex">
              ...
              <div>
                {line.includes("=") && !line.includes("print") ? (
                  <>
                    <span className="text-cyan-400">fruits</span>
                    <span className="text-yellow-400"> = </span>
                    <span className="text-gray-300">[</span>
                    <span className="text-green-400">"Apple", "Banana", "Mango"</span>
                    <span className="text-gray-300">]</span>
                  </>
                ) : line.includes("print") ? (
                  <>
        ...
        <div className="px-4 pb-3 bg-gray-900">
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-gray-500 font-bold">fruits =</span>
            <div className="flex gap-1.5">
              {fruitsInList.map((f, i) => (
                <motion.div key={i} initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                  transition={{ type: "spring", stiffness: 300 }}
                  className="px-2 py-1 bg-cyan-900/30 border border-cyan-500/30 rounded-lg text-[10px] font-mono text-cyan-400">
                  {f}
                </motion.div>
              ))}
            </div>
        ...
        {phase === "done" && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-bold text-green-700">Python stores all three fruits inside one List. The print() function displays every item together!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export function Ch5Basic({
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      else if (q.includes("Square Brackets") || q.includes("square")) { a = "List লেখার জন্য square brackets [ ] ব্যবহার করি। ভেতরে items গুলোকে comma দিয়ে আলাদা করি। যেমন: [\"Apple\", \"Banana\"]"; }
      else if (q.includes("Variable") || q.includes("variable")) { a = "Variable একটি মান ধারণ করে, কিন্তু List একাধিক মান ধারণ করে। Variable: x = 5, List: fruits = [\"Apple\", \"Banana\"]"; }
      else if (q.includes("একাধিক") || q.includes("multiple")) { a = "List-এ যত খুশি item রাখা যায়! শুধু items গুলোকে comma দিয়ে আলাদা করো। যেমন: [1, 2, 3] বা [\"a\", \"b\", \"c\"]"; }
      else if (q.includes("print") || q.includes("দেখবো")) { a = "print() দিয়ে List দেখাতে হয়। print(fruits) লিখলে Python List-এর সব items একসাথে দেখাবে।"; }
      else { a = "ভালো প্রশ্ন! Python List হলো একাধিক জিনিস একসাথে রাখার সবচেয়ে সহজ উপায়। নিচের suggested questions দেখো!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">List কী?</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              <span className="font-bold text-cyan-600">List</span> হলো একটি container যেখানে <span className="font-bold text-purple-600">একাধিক জিনিস</span> একসাথে রাখা যায়!
            </p>
            <p className="text-gray-700 text-[15px] leading-relaxed mt-2">
              অনেকগুলো Variable বানানোর বদলে,
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🎒 School Bag Example</p>
              <SchoolBagAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">📦 List Learning Flow</p>
              <ListFlowAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> List
                হলো একাধিক item একসাথে রাখার সবচেয়ে সহজ উপায়।
                Square brackets [ ] দিয়ে লেখা হয়!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">List আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "🛒", label: "Shopping List", desc: "কেনাকাটার তালিকা", code: "Milk, Bread, Eggs" },
              { icon: "🍎", label: "Fruit Basket", desc: "ফলের ঝুড়ি", code: "Apple, Banana, Mango" },
              { icon: "🎒", label: "School Bag", desc: "স্কুল ব্যাগ", code: "Book, Pencil, Notebook" },
              { icon: "🎵", label: "Playlist", desc: "গানের তালিকা", code: "Song1, Song2, Song3" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🛒 Shopping List Example</p>
              <div className="flex items-center justify-center gap-4">
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                ...
                List ঠিক যেমন আমরা কেনাকাটার তালিকায় items রাখি —
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: List তৈরি করো!</p>
                  <p className="text-xs text-gray-500 mt-1">lists.py ফাইলে List তৈরি করো</p>
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-cyan-400">fruits</span> <span className="text-yellow-400">=</span> <span className="text-gray-300">[</span><span className="text-green-400">"Apple", "Banana", "Mango"</span><span className="text-gray-300">]</span></p>
                <p></p>
                <p><span className="text-purple-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">fruits</span><span className="text-gray-300">)</span></p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "fruits নামে একটি List তৈরি করো।", code: 'fruits = ["Apple", "Banana", "Mango"]' },
              { num: 2, text: "তিনটি fruit item রাখো।", code: "# Apple, Banana, Mango" },
              { num: 3, text: "print() দিয়ে List দেখাও।", code: "print(fruits)" },
            ].map((step, i) => (
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-rose-800 via-pink-800 to-purple-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 5: Lists - Basic</p>
                  <p className="text-white/50 text-xs mt-1">Learn about Python Lists</p>
                </div>
              </div>
            </div>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🏆</span>
              <p className="text-sm font-bold text-purple-800">Goal: List তৈরি করো!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-cyan-400">fruits = ["Apple", "Banana", "Mango"]</p>
              <p className="text-purple-400">print(fruits)</p>
            </div>
          </motion.div>
            ...
            <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
              ...
              <div className="flex bg-gray-900">
                ...
                <textarea value={editorCode} onChange={(e) => { setEditorCode(e.target.value); setShowEncourage(false); }}
                  placeholder={'fruits = ["Apple", "Banana", "Mango"]\n\nprint(fruits)'}
            ...
            {solutionOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="overflow-hidden">
                <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-md">
                  ...
                  <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
                    {[
                      { parts: [{ c: "text-cyan-400", t: "fruits" }, { c: "text-yellow-400", t: " = " }, { c: "text-gray-300", t: "[" }, { c: "text-green-400", t: '"Apple", "Banana", "Mango"' }, { c: "text-gray-300", t: "]" }] },
                      { parts: [] },
                      { parts: [{ c: "text-purple-400", t: "print" }, { c: "text-gray-300", t: "(" }, { c: "text-cyan-400", t: "fruits" }, { c: "text-gray-300", t: ")" }] },
                    ].map((item, i) => (
                      <div key={i} className="flex">
                  ...
                  <div className="px-4 pb-4 bg-gray-900">
                    <div className="p-3 bg-gray-800 rounded-lg font-mono text-xs">
                      ...
                      <span className="text-green-400 font-bold">['Apple', 'Banana', 'Mango']</span>
                    </div>
                  </div>
                </div>
              </motion.div>
            ...
            <div className="h-72 overflow-y-auto p-4 space-y-3">
              {aiMessages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-xs text-gray-500 max-w-[240px] mb-4 leading-relaxed">Lists সম্পর্কে যেকোনো প্রশ্ন করো! নিচের যেকোনো প্রশ্নে ক্লিক করো।</p>
