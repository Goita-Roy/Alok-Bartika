...
const LESSON_ID = "ch5-todo";
const SOLUTION_CODE = `todo = ["Study Python", "Finish Homework", "Drink Water", "Clean Room"]\n\nprint(todo)`;

const SUGGESTED_QUESTIONS = [
  "List কী?",
  "To-Do List কী?",
  "Square Brackets `[ ]` কেন ব্যবহার করি?",
  "এক List-এ অনেক Task রাখা যায় কীভাবে?",
...
const HINTS = [
  "Create a Variable named `todo`।",
  "Square brackets `[ ]` ব্যবহার করো।",
  "প্রতিটি task-কে comma দিয়ে আলাদা করো।",
...
/* ── Today's Tasks Animation ── */
function TodayTasksAnimation() {
  const [step, setStep] = useState(-1);

  const items = [
    { icon: "📝", label: "Today's Tasks", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "📚", label: "Study Python", color: "text-cyan-400", bg: "bg-cyan-900/30 border-cyan-500/50" },
    { icon: "✍️", label: "Finish HW", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "💧", label: "Drink Water", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "🛏️", label: "Clean Room", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
    { icon: "✅", label: "All Done!", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {step < items.length - 1 ? "▶ পরবর্তী ধাপ" : "🔄 পুনরায় শুরু করুন"}
      </button>
        ...
        {step >= 5 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              ...
              <p className="text-sm font-bold text-green-400">All tasks organized — just like a Python List!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
/* ── Todo Learning Flow ── */
function TodoFlowAnimation() {
  const [step, setStep] = useState(-1);

  const steps = [
    { icon: "📝", label: "Daily Tasks", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    ...
    { icon: "⬇️", label: "", color: "text-gray-400", bg: "bg-gray-800/30 border-gray-500/50" },
    { icon: "📋", label: "Easy to Organize", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
    { icon: "🔄", label: "Easy to Update", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "🧠", label: "Easy to Remember", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
  ];
      ...
      <AnimatePresence>
        {step >= 6 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              <span className="text-lg">🎉</span>
              <p className="text-sm font-bold text-green-400">Lists make task management easy!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function SquareBracketsIntro() {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {show ? "🔄 পুনরায় দেখুন" : "▶ Square Brackets দেখান"}
      </button>
...
function ExampleSection() {
  ...
  const fullCode = 'todo = ["Study Python", "Finish Homework", "Drink Water", "Clean Room"]\n\nprint(todo)';
  ...
  const startDemo = useCallback(() => {
    ...
    const typeInterval = setInterval(() => {
      if (idx < fullCode.length) {
        ...
        setTimeout(() => {
          setPhase("running");
          const tasks = ["Study Python", "Finish Homework", "Drink Water", "Clean Room"];
          let tIdx = 0;
          const addTask = setInterval(() => {
            if (tIdx < tasks.length) {
              setChecklist(prev => [...prev, tasks[tIdx]]);
              tIdx++;
            } else {
              clearInterval(addTask);
              setTimeout(() => {
                setOutputLines(["['Study Python', 'Finish Homework', 'Drink Water', 'Clean Room']"]);
                setPhase("done");
              }, 500);
            }
          }, 600);
  ...
  return (
    <div className="space-y-4">
      <button onClick={startDemo} disabled={phase === "typing" || phase === "running"}
        ...
        {phase === "idle" || phase === "done" ? "▶ ডেমো চালান" : phase === "typing" ? "⌨️ টাইপ করা হচ্ছে..." : "⏳ টাস্ক যোগ হচ্ছে..."}
      </button>
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-gray-400 text-xs font-mono">todo.py</span>
          </div>
        </div>
              ...
              <div>
                {line.includes("=") && !line.includes("print") ? (
                  <>
                    ...
                    <span className="text-green-400">"Study Python", "Finish Homework", "Drink Water", "Clean Room"</span>
                    <span className="text-gray-300">]</span>
                  </>
                ) : line.includes("print") ? (
                  <>
          ...
          <div className="space-y-1.5">
            {checklist.map((task, i) => (
              ...
              >
                ...
                <span className="text-cyan-400 font-mono text-[11px]">{task}</span>
              </motion.div>
            ))}
            {checklist.length === 0 && (
              <p className="text-[10px] text-gray-500 font-mono">☐ No tasks yet...</p>
            )}
          </div>
        </div>
        ...
        {phase === "done" && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-bold text-green-700">Python stores every task inside one List. The print() function displays the complete To-Do List!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export function Ch5Todo({
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      else if (q.includes("To-Do") || q.includes("todo")) { a = "To-Do List হলো আমাদের দৈনিক কাজের তালিকা। Python-এ আমরা সব task একটি List-এ রাখি — যেমন: todo = [\"Study\", \"Homework\"]"; }
      else if (q.includes("Square Brackets") || q.includes("square")) { a = "List লেখার জন্য square brackets [ ] ব্যবহার করি। ভেতরে tasks গুলোকে comma দিয়ে আলাদা করি। যেমন: [\"Study Python\", \"Drink Water\"]"; }
      else if (q.includes("অনেক") || q.includes("many") || q.includes("multiple")) { a = "List-এ যত খুশি task রাখা যায়! শুধু tasks গুলোকে comma দিয়ে আলাদা করো। যেমন: [\"Task1\", \"Task2\", \"Task3\"]"; }
      else if (q.includes("print") || q.includes("দেখবো")) { a = "print() দিয়ে List দেখাতে হয়। print(todo) লিখলে Python List-এর সব tasks একসাথে দেখাবে।"; }
      else { a = "ভালো প্রশ্ন! To-Do List হলো দৈনিক কাজের তালিকা — ঠিক যেমন আমরা প্রতিদিন tasks লিখি। নিচের suggested questions দেখো!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">To-Do List কী?</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              <span className="font-bold text-cyan-600">List</span> দিয়ে আমরা <span className="font-bold text-purple-600">দৈনিক কাজ</span> একসাথে সাজাতে পারি!
            </p>
            <p className="text-gray-700 text-[15px] leading-relaxed mt-2">
              আলাদা Variable বানানোর বদলে,
              Python-এ <span className="font-bold text-amber-600">একটি List</span>-এ সব task রাখা যায়।
            </p>
          </motion.div>

          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">📝 Today's Tasks</p>
              <TodayTasksAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">📦 To-Do List Flow</p>
              <TodoFlowAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> To-Do
                List হলো আমাদের দৈনিক কাজের তালিকা —
                ঠিক যেমন আমরা প্রতিদিন tasks লিখি।
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">To-Do List আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "📝", label: "Daily Planner", desc: "দৈনিক পরিকল্পনা", code: "Study, Exercise, Read" },
              { icon: "📚", label: "Homework Checklist", desc: "বাড়ির কাজের তালিকা", code: "Math, English, Science" },
              { icon: "🛒", label: "Shopping Tasks", desc: "কেনাকাটার কাজ", code: "Milk, Bread, Eggs" },
              { icon: "🎉", label: "Weekend Plans", desc: "সাপ্তাহিক পরিকল্পনা", code: "Park, Movie, Game" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">📝 Daily Planner Example</p>
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
                    className="px-4 py-3 bg-red-900/30 border border-red-500/30 rounded-xl">
                    <p className="text-red-400 font-mono text-sm">Remember each task</p>
                  </motion.div>
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                ...
                List ঠিক যেমন আমরা দৈনিক তালিকা লিখি —
                তেমনি Python-এ tasks রাখি!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: To-Do List তৈরি করো!</p>
                  <p className="text-xs text-gray-500 mt-1">todo.py ফাইলে List তৈরি করো</p>
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-cyan-400">todo</span> <span className="text-yellow-400">=</span> <span className="text-gray-300">[</span><span className="text-green-400">"Study Python", "Finish Homework", "Drink Water", "Clean Room"</span><span className="text-gray-300">]</span></p>
                <p></p>
                <p><span className="text-purple-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">todo</span><span className="text-gray-300">)</span></p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "todo নামে একটি List তৈরি করো।", code: 'todo = ["Study Python", "Finish Homework", "Drink Water", "Clean Room"]' },
              { num: 2, text: "চারটি daily task রাখো।", code: "# Study Python, Finish Homework, Drink Water, Clean Room" },
              { num: 3, text: "print() দিয়ে List দেখাও।", code: "print(todo)" },
            ].map((step, i) => (
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-rose-800 via-pink-800 to-purple-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 5: To-Do List</p>
                  <p className="text-white/50 text-xs mt-1">Build a to-do list with Python Lists</p>
                </div>
              </div>
            </div>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🏆</span>
              <p className="text-sm font-bold text-purple-800">Goal: To-Do List তৈরি করো!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-cyan-400">todo = ["Study Python", "Finish Homework", "Drink Water", "Clean Room"]</p>
              <p className="text-purple-400">print(todo)</p>
            </div>
          </motion.div>
            ...
            <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
              ...
              <div className="flex bg-gray-900">
                ...
                <textarea value={editorCode} onChange={(e) => { setEditorCode(e.target.value); setShowEncourage(false); }}
                  placeholder={'todo = ["Study Python", "Finish Homework", "Drink Water", "Clean Room"]\n\nprint(todo)'}
            ...
            {solutionOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="overflow-hidden">
                <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-md">
                  ...
                  <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
                    {[
                      { parts: [{ c: "text-cyan-400", t: "todo" }, { c: "text-yellow-400", t: " = " }, { c: "text-gray-300", t: "[" }, { c: "text-green-400", t: '"Study Python", "Finish Homework", "Drink Water", "Clean Room"' }, { c: "text-gray-300", t: "]" }] },
                      { parts: [] },
                      { parts: [{ c: "text-purple-400", t: "print" }, { c: "text-gray-300", t: "(" }, { c: "text-cyan-400", t: "todo" }, { c: "text-gray-300", t: ")" }] },
                    ].map((item, i) => (
                      <div key={i} className="flex">
                  ...
                  <div className="px-4 pb-4 bg-gray-900">
                    <div className="p-3 bg-gray-800 rounded-lg font-mono text-xs">
                      ...
                      <span className="text-green-400 font-bold">['Study Python', 'Finish Homework', 'Drink Water', 'Clean Room']</span>
                    </div>
                  </div>
                </div>
              </motion.div>
            ...
            <div className="h-72 overflow-y-auto p-4 space-y-3">
              {aiMessages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-xs text-gray-500 max-w-[240px] mb-4 leading-relaxed">To-Do List সম্পর্কে যেকোনো প্রশ্ন করো! নিচের যেকোনো প্রশ্নে ক্লিক করো।</p>
