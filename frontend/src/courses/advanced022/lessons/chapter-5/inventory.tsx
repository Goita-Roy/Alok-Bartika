...
const LESSON_ID = "ch5-inventory";
const SOLUTION_CODE = `inventory = ["Laptop", "Keyboard", "Mouse", "Headphones"]\n\nprint(inventory)`;

const SUGGESTED_QUESTIONS = [
  "List কী?",
  "Inventory কী?",
  "Square Brackets `[ ]` কেন ব্যবহার করি?",
  "এক List-এ অনেক Item রাখা যায় কীভাবে?",
  "`print()` দিয়ে পুরো Inventory কীভাবে দেখবো?",
];
...
function InventoryBoxAnimation() {
  const [step, setStep] = useState(-1);

  const items = [
    { icon: "📦", label: "Inventory", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    { icon: "🖥️", label: "Laptop", color: "text-cyan-400", bg: "bg-cyan-900/30 border-cyan-500/50" },
    { icon: "⌨️", label: "Keyboard", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "🖱️", label: "Mouse", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
    { icon: "🎧", label: "Headphones", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
    { icon: "✅", label: "All Stored!", color: "text-green-400", bg: "bg-green-900/30 border-green-500/50" },
  ];
        ...
        {step >= 5 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              ...
              <p className="text-sm font-bold text-green-400">All items stored — just like a Python List!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
/* ── Inventory Learning Flow ── */
function InventoryFlowAnimation() {
  const [step, setStep] = useState(-1);

  const steps = [
    { icon: "📝", label: "Many Items", color: "text-blue-400", bg: "bg-blue-900/30 border-blue-500/50" },
    ...
    { icon: "🔍", label: "Easy to Find", color: "text-amber-400", bg: "bg-amber-900/30 border-amber-500/50" },
    { icon: "⚙️", label: "Easy to Manage", color: "text-purple-400", bg: "bg-purple-900/30 border-purple-500/50" },
  ];
      ...
      <AnimatePresence>
        {step >= 6 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/30 border border-green-500/30 rounded-xl">
              <span className="text-lg">🎉</span>
              <p className="text-sm font-bold text-green-400">Lists make inventory management easy!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-center gap-4">
        ...
        >
          ...
          >
            inventory items inside brackets
          </motion.p>
        </motion.div>
...
function ExampleSection() {
  ...
  const fullCode = 'inventory = ["Laptop", "Keyboard", "Mouse", "Headphones"]\n\nprint(inventory)';
  ...
  const startDemo = useCallback(() => {
    ...
    const typeInterval = setInterval(() => {
      if (idx < fullCode.length) {
        ...
        setTimeout(() => {
          setPhase("running");
          const items = ["Laptop", "Keyboard", "Mouse", "Headphones"];
          let itemIdx = 0;
          const addItem = setInterval(() => {
            if (itemIdx < items.length) {
              setShelfItems(prev => [...prev, items[itemIdx]]);
              itemIdx++;
            } else {
              clearInterval(addItem);
              setTimeout(() => {
                setOutputLines(["['Laptop', 'Keyboard', 'Mouse', 'Headphones']"]);
                setPhase("done");
              }, 500);
            }
          }, 600);
  ...
  return (
    <div className="space-y-4">
      <button onClick={startDemo} disabled={phase === "typing" || phase === "running"}
        ...
        {phase === "idle" || phase === "done" ? "▶ ডেমো চালান" : phase === "typing" ? "⌨️ টাইপ করা হচ্ছে..." : "📦 আইটেম যোগ হচ্ছে..."}
      </button>
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
        <div className="flex items-center justify-between px-4 py-2.5 bg-gray-800">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-gray-400 text-xs font-mono">inventory.py</span>
          </div>
        </div>
              ...
              <div>
                {line.includes("=") && !line.includes("print") ? (
                  <>
                    ...
                    <span className="text-green-400">"Laptop", "Keyboard", "Mouse", "Headphones"</span>
                    <span className="text-gray-300">]</span>
                  </>
                ) : line.includes("print") ? (
                  <>
          ...
          <div className="space-y-1.5">
            {shelfItems.map((item, i) => (
              ...
              >
                ...
                <span className="text-cyan-400 font-mono text-[11px]">{item}</span>
              </motion.div>
            ))}
            {shelfItems.length === 0 && (
              <p className="text-[10px] text-gray-500 font-mono">📦 Empty shelf...</p>
            )}
          </div>
        </div>
        ...
        {phase === "done" && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-bold text-green-700">Python stores every inventory item inside one List. The print() function displays the complete inventory!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export function Ch5Inventory({
  ...
  const aiSend = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      else if (q.includes("Inventory") || q.includes("inventory")) { a = "Inventory হলো সব item একসাথে সংরক্ষণের তালিকা। Python-এ আমরা inventory নামে একটি List তৈরি করি — যেমন: inventory = [\"Laptop\", \"Mouse\"]"; }
      else if (q.includes("Square Brackets") || q.includes("square")) { a = "List লেখার জন্য square brackets [ ] ব্যবহার করি। ভেতরে items গুলোকে comma দিয়ে আলাদা করি। যেমন: [\"Laptop\", \"Mouse\"]"; }
      else if (q.includes("অনেক") || q.includes("many") || q.includes("multiple")) { a = "List-এ যত খুশি item রাখা যায়! শুধু items গুলোকে comma দিয়ে আলাদা করো। যেমন: [\"Item1\", \"Item2\", \"Item3\"]"; }
      else if (q.includes("print") || q.includes("দেখবো")) { a = "print() দিয়ে List দেখাতে হয়। print(inventory) লিখলে Python List-এর সব items একসাথে দেখাবে।"; }
      else { a = "ভালো প্রশ্ন! Inventory হলো সব item একসাথে সংরক্ষণের তালিকা। নিচের suggested questions দেখো!"; }
    ...
    <div className="space-y-8 pb-12">
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6 space-y-6">
          <motion.div variants={FADE_UP}>
            <h3 className="text-lg font-bold text-gray-900 mb-3">Inventory কী?</h3>
            <p className="text-gray-700 text-[15px] leading-relaxed">
              <span className="font-bold text-cyan-600">List</span> দিয়ে আমরা <span className="font-bold text-purple-600">সব item</span> একসাথে সংরক্ষণ করতে পারি!
            </p>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">📦 Inventory Example</p>
              <InventoryBoxAnimation />
            </div>
          </motion.div>

          <motion.div variants={FADE_UP}>
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">📦 Inventory Flow</p>
              <InventoryFlowAnimation />
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP}>
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800 leading-relaxed">
                <span className="font-bold">মনে রাখো:</span> Inventory
                হলো সব item একসাথে সংরক্ষণের তালিকা —
                ঠিক যেমন একটি দোকানে সব product তালিকাভুক্ত থাকে।
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.p variants={FADE_UP} className="text-sm text-gray-500 mb-5">Inventory আমাদের দৈনন্দিন জীবনেও ঘটে!</motion.p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { icon: "🏪", label: "Shop Inventory", desc: "দোকানের তালিকা", code: "Laptop, Mouse, USB" },
              { icon: "📚", label: "Library Books", desc: "গ্রন্থাগারের বই", code: "Math, Science, English" },
              { icon: "🎮", label: "Game Items", desc: "গেমের জিনিস", code: "Sword, Shield, Potion" },
              { icon: "🏫", label: "School Equipment", desc: "স্কুলের উপকরণ", code: "Desk, Chair, Board" },
            ].map((item, i) => (
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-5">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-4 text-center">🏪 Shop Inventory Example</p>
              <div className="flex items-center justify-center gap-4">
          ...
          <motion.div variants={FADE_UP} className="mt-5">
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-teal-800 leading-relaxed">
                ...
                List ঠিক যেমন দোকানে সব product তালিকাভুক্ত থাকে —
                তেমনি Python-এ items রাখি!
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={STAGGER} className="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden">
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5">
            <div className="bg-gradient-to-r from-orange-50 to-rose-50 border-2 border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-extrabold text-gray-900 text-lg">Mission: Inventory List তৈরি করো!</p>
                  <p className="text-xs text-gray-500 mt-1">inventory.py ফাইলে List তৈরি করো</p>
              ...
              <div className="bg-gray-900 rounded-xl p-4 font-mono text-sm space-y-1">
                <p><span className="text-cyan-400">inventory</span> <span className="text-yellow-400">=</span> <span className="text-gray-300">[</span><span className="text-green-400">"Laptop", "Keyboard", "Mouse", "Headphones"</span><span className="text-gray-300">]</span></p>
                <p></p>
                <p><span className="text-purple-400">print</span><span className="text-gray-300">(</span><span className="text-cyan-400">inventory</span><span className="text-gray-300">)</span></p>
              </div>
            </div>
          </motion.div>
          ...
          <div className="space-y-3">
            {[
              { num: 1, text: "inventory নামে একটি List তৈরি করো।", code: 'inventory = ["Laptop", "Keyboard", "Mouse", "Headphones"]' },
              { num: 2, text: "চারটি inventory item রাখো।", code: "# Laptop, Keyboard, Mouse, Headphones" },
              { num: 3, text: "print() দিয়ে List দেখাও।", code: "print(inventory)" },
            ].map((step, i) => (
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-rose-800 via-pink-800 to-purple-800">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white/80 text-sm font-medium mt-3">Chapter 5: Inventory</p>
                  <p className="text-white/50 text-xs mt-1">Build an inventory list with Python</p>
                </div>
              </div>
            </div>
        ...
        <div className="px-6 py-6">
          <motion.div variants={FADE_UP} className="mb-5 bg-purple-50 border border-purple-200 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🏆</span>
              <p className="text-sm font-bold text-purple-800">Goal: Inventory List তৈরি করো!</p>
            </div>
            <div className="bg-gray-900 rounded-xl p-3 font-mono text-sm space-y-1">
              <p className="text-cyan-400">inventory = ["Laptop", "Keyboard", "Mouse", "Headphones"]</p>
              <p className="text-purple-400">print(inventory)</p>
            </div>
          </motion.div>
            ...
            <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md">
              ...
              <div className="flex bg-gray-900">
                ...
                <textarea value={editorCode} onChange={(e) => { setEditorCode(e.target.value); setShowEncourage(false); }}
                  placeholder={'inventory = ["Laptop", "Keyboard", "Mouse", "Headphones"]\n\nprint(inventory)'}
            ...
            {solutionOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="overflow-hidden">
                <div className="mt-4 rounded-2xl overflow-hidden border border-gray-200 shadow-md">
                  ...
                  <div className="bg-gray-900 p-4 font-mono text-sm space-y-1">
                    {[
                      { parts: [{ c: "text-cyan-400", t: "inventory" }, { c: "text-yellow-400", t: " = " }, { c: "text-gray-300", t: "[" }, { c: "text-green-400", t: '"Laptop", "Keyboard", "Mouse", "Headphones"' }, { c: "text-gray-300", t: "]" }] },
                      { parts: [] },
                      { parts: [{ c: "text-purple-400", t: "print" }, { c: "text-gray-300", t: "(" }, { c: "text-cyan-400", t: "inventory" }, { c: "text-gray-300", t: ")" }] },
                    ].map((item, i) => (
                      <div key={i} className="flex">
                  ...
                  <div className="px-4 pb-4 bg-gray-900">
                    <div className="p-3 bg-gray-800 rounded-lg font-mono text-xs">
                      ...
                      <span className="text-green-400 font-bold">['Laptop', 'Keyboard', 'Mouse', 'Headphones']</span>
                    </div>
                  </div>
                </div>
              </motion.div>
            ...
            <div className="h-72 overflow-y-auto p-4 space-y-3">
              {aiMessages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-xs text-gray-500 max-w-[240px] mb-4 leading-relaxed">Inventory সম্পর্কে যেকোনো প্রশ্ন করো! নিচের যেকোনো প্রশ্নে ক্লিক করো।</p>
