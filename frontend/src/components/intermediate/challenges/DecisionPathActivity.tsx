...
export function DecisionPathActivity() {
  ...
  return (
    <section ref={ref} className="py-20 px-4 bg-gradient-to-br from-amber-50 to-yellow-50 dark:from-gray-900 dark:to-slate-900">
      <div className="max-w-3xl mx-auto w-full">
        ...
        <Card>
          ...
          <div className="relative bg-gradient-to-b from-green-100 to-green-50 dark:from-slate-700 dark:to-slate-800 rounded-xl p-8 mb-6 min-h-[280px] overflow-hidden">
            ...
            {/* Center condition */}
            <motion.div
              className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 bg-white dark:bg-gray-800 border-4 border-emerald-500 rounded-xl px-4 py-3 shadow-lg text-center"
              initial={{ scale: 0 }}
              animate={isVisible ? { scale: 1 } : {}}
              transition={{ type: 'spring', delay: 0.3 }}
            >
              <p className="font-bold text-sm text-emerald-500">রাস্তা যদি ভেজা থাকে</p>
            </motion.div>
            ...
            {showResult && path === 'left' && (
              <motion.div key="left" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-center mb-6">
                <Card className="bg-red-50 dark:bg-red-900/20 border-red-300">
                  <p className="text-xl font-bold text-red-500">ছাতা ছাড়া যাও → তুমি ভিজে যাবে ❌</p>
                </Card>
              </motion.div>
            )}
            {showResult && path === 'right' && (
              <motion.div key="right" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-center mb-6">
                <Card className="bg-green-50 dark:bg-green-900/20 border-green-300">
                  <p className="text-xl font-bold text-green-500">ছাতা নাও → তুমি ভিজবে না ✅</p>
                </Card>
              </motion.div>
            )}
          </AnimatePresence>
