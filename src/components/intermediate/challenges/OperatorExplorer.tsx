        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {categories.map((cat, index) => (
            <motion.div key={cat.id} initial={{ opacity: 0, y: 20 }} animate={isVisible ? { opacity: 1, y: 0 } : {}} transition={{ delay: index * 0.1 }}>
              <div 
                className={`cursor-pointer rounded-2xl border-2 p-6 transition-all duration-300 ${cat.borderColor} ${activeCategory === cat.id ? 'ring-4 ring-emerald-500/30 scale-105' : 'hover:scale-105 hover:shadow-lg'} bg-white dark:bg-gray-800`}
                onClick={() => setActiveCategory(activeCategory === cat.id ? null : cat.id)}
              >
                <div className="flex flex-col items-center text-center">
                  <div className="text-5xl mb-4">{cat.icon}</div>
                  <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">{cat.title}</h3>
                </div>
              </div>
